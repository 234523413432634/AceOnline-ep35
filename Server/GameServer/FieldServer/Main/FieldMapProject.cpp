...
//	SQLFreeStmt(hstmt, SQL_CLOSE);
	...
	RETCODE		ret;
	SQLINTEGER	arrCB[9] = {SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS, SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS};	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 8로 수정
	UINT		BuildingIndex;
	...
	for(int i=0; i < vectBuildingIndexList.size(); i++)
	{
		...
		if (itr_shop != m_mapShopInfo.end())
		{
			...
			**************************************************************************/
			...
			arrCB[1]=arrCB[2]=arrCB[3]=arrCB[4]=arrCB[5]=arrCB[6]=arrCB[7]=SQL_NTS;		// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 초기화 추가

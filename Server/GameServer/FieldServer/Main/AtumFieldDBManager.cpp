...
BOOL CAtumFieldDBManager::LoadInfluenceWarData(BYTE *o_pOwnerInflOfConflictArea, CODBCStatement *i_pODBCStmt, CInflWarManager *i_pInflWarManager)
{
	*o_pOwnerInflOfConflictArea		= INFLUENCE_TYPE_UNKNOWN;		// 2006-07-12 by cmkwon
	...
	RETCODE					ret;
	SQLINTEGER				arrCB[13] = {SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS, SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS, SQL_NTS,SQL_NTS,SQL_NTS};	// 2008-04-10 by dhjin, 모선전 정보 표시 기획안 - 
...
//		pTmpNewItemGeneral->ItemInfo = m_pFieldIOCP8->GetItemInfo(pTmpNewItemGeneral->ItemNum);	// ItemInfo 할당
//
...
// 2007-01-30 by cmkwon, CAtumFieldDBManager::QP_NewGetStoreItem()함수로 대체함
...
//
//	ITEM_GENERAL	ItemGeneral(NULL);		// 스킬 이외의 아이템용
//	ITEM_SKILL		ItemSkill(NULL);	// 스킬 용
//	RETCODE			ret;
//	SQLINTEGER		pcbNTS = SQL_NTS;
//	SQLINTEGER		arrCB[17] = {SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS, SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,
//								SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS,SQL_NTS, SQL_NTS,SQL_NTS};
//	SQL_TIMESTAMP_STRUCT tmpTimeStamp;
//	SQL_TIMESTAMP_STRUCT tmpUseTimeStamp;		// 2006-11-17 by dhjin, 2차 스킬 추가로 인한 스킬 사용 시간
//
//	/*[Stored Query Definition]************************************************
//	--!!!!
//	-- Name:
...
//	SQLBindCol(hstmt, 14, SQL_C_LONG, &ItemGeneral.ColorCode, 0,			&arrCB[14]);		// 2005-11-21 by cmkwon, 추가됨
...
//	{
//		pFieldIOCPSocket->m_ItemManager.ResetAllItemGeneralsInStore();			// 2006-09-19 by dhjin, 창고 초기화
...
//
//// 2005-01-17 by cmkwon(dhkwon), 필요 없음 
////	////////////////////////////////////////////////////////////////////////
////	// 창고 진입 혹은 캐릭터 아이템 시작 시점을 명시하기 위해 더미로 생성
...
//		}
//
//		// check: store item의 kind는 DB에서는 없애고, ItemInfo에 설정된 값으로 세팅함, 20030902, kelovon
...
//				{// 2006-10-11 by cmkwon, ITEMKIND_ACCESSORY_TIMELIMIT는 장착 아이템으로 처리
...
//			SQLFreeStmt(hstmt, SQL_CLOSE);	// clean up
//
//			pFieldIOCPSocket->m_ItemManager.InsertItemBaseByItemNum(7009210, 10, IUT_MGAME_EVENT);			// 도시 귀환 포탈(10개)
//			pFieldIOCPSocket->m_ItemManager.InsertItemBaseByItemNum(7003070, 30, IUT_MGAME_EVENT);			// A급 쉴드킷(30개)
//			pFieldIOCPSocket->m_ItemManager.InsertItemBaseByItemNum(7003060, 30, IUT_MGAME_EVENT);			// A급 수리킷(30개)
//			pFieldIOCPSocket->m_ItemManager.InsertItemBaseByItemNum(7000022, 50000, IUT_MGAME_EVENT);		// SPI(5만)
...
//	{// 2006-10-16 by cmkwon, 현재 게임 중인 캐릭터 일때만 스킬을 로딩한다.
//
//		/*[Stored Query Definition]************************************************
...
//		SQLBindCol(hstmt, 7, SQL_C_TIMESTAMP, &tmpUseTimeStamp, 0,				&arrCB[7]);		// 2006-11-17 by dhjin, 2차 스킬로 인하여 스킬 사용 시간 추가
//		// 2006-12-05 by dhjin
//		// initialize to 0x00
...
//
//			//////////////////////////////////////////////////////////////////////////
//			// 2006-11-17 by dhjin, 2차 스킬로 인한 스킬 사용 시간 추가
...
//// 2006-12-20 by cmkwon, 여단 창고 아이템에 문제가 발생함, 일단 제거함
...
//	SQLFreeStmt(hstmt, SQL_CLOSE);
//
//	mtE.auto_unlock_cancel();							// 2006-12-18 by cmkwon, AutoLock을 해제한다 
//
//#ifdef _DEBUG
//// 2005-02-02 by cmkwon, 필요없음
		...
		case ITEMKIND_PET_ITEM :
			{
				...
				// 펫의 레벨 Data 유효 체크 후 'ShapeItemNum'값 임의 설정. ( 캐릭터 선택창을 위해 필드 Source Index값으로 설정. )
				if ( a_psPetLvData )
...
//	SQLBindParameter(hstmt, 6, SQL_PARAM_INPUT, SQL_C_SBIGINT, SQL_BIGINT, 0, 0, &pItem->ScarcityNumber, 0, NULL);
	...
	*/
	...
	if ( pItem->ItemInfo->Kind == ITEMKIND_PET_ITEM )
	{
		...
		SQLBindCol(hstmt, 8, SQL_C_LONG, &(pPetCurData->Stamina),			0, &arrcb[7]); // 2010-12-03 by shcho 빠진부분 수정 - 인자값 빠진부분 수정
...
// 
// 	// pFromSocket의 아이템을 pToSocket의 아이템으로 옮김
...
// 
// 			///////////////////////////////////////////////////////////////////////////////
// 			// 2005-09-13 by cmkwon, 최대(20억개) 개수 체크
// 			INT64 tmCurrentCount = tradeItem.Count;
// 			ITEM_GENERAL *pToItem = pToSocket->m_ItemManager.GetFirstItemGeneralByItemNum(pItem->ItemNum);	// 받는 사람의 아이템
...
// 
// 			// 만약 새로 삽입되었다면 처리함, MoveTradeItemField()에서는 처리하지 않는다.
// 			// 아이템을 받는 쪽 처리
// 			if (pToItem == NULL)
// 			{
// 				// 삽입
...
// 				pTmpCountable->ScarcityNumber		= pItem->ScarcityNumber;	// check: 희귀도 기획 정해지면 수정가능성 있음
// 				pTmpCountable->ItemWindowIndex		= POS_INVALID_POSITION; // check: 혹은 POS_ITEMWINDOW_OFFSET
// 				pTmpCountable->NumOfEnchants		= pItem->NumOfEnchants;	// 거래이기 때문에 NumOfEnchants를 꼭 할당한다!
// 				pTmpCountable->PrefixCodeNum		= pItem->PrefixCodeNum;
// 				pTmpCountable->SuffixCodeNum		= pItem->SuffixCodeNum;
// 				pTmpCountable->CurrentEndurance		= pItem->CurrentEndurance;			// 거래이기 때문에 남은 endurance를 꼭 할당한다!
// 
// 				// 캐릭터에게 삽입
...
// 
// 				// Send log
// 				CAtumLogSender::SendLogMessageITEMTradeRecv(pToSocket, pTmpCountable, tradeItem.Count, &pFromSocket->m_character);	// 아이템 거래: 받기, countable, 추가되는 경우
// 			}
// 			else
// 			{
// 				// 업데이트
...
// 
// 				// Send log
// 				CAtumLogSender::SendLogMessageITEMTradeRecv(pToSocket, pToItem, tradeItem.Count, &pFromSocket->m_character);	// 아이템 거래: 받기, countable, 개수만 늘어나는 경우
// 			}
// 
// 			// 아이템을 주는 쪽 처리
...
// 
// 				// Send log
// 				CAtumLogSender::SendLogMessageITEMTradeSend(pFromSocket, pItem, tradeItem.Count, &pToSocket->m_character);	// 아이템 거래: 주기, countable, 개수만 줄어드는 경우
// 			}
// 			else
// 			{
// 				// Send log
...
// 				CAtumLogSender::SendLogMessageITEMTradeSend(pFromSocket, pItem, tradeItem.Count, &pToSocket->m_character);	// 아이템 거래: 주기, countable, 삭제되는 경우
// 
// 				// 삭제
...
// 		{
// 			///////////////////////////////////////////////////////////////////
// 			// Trade Item이 Countable Item 인 경우
// 			///////////////////////////////////////////////////////////////////
// 
// 			// check: MoveTradeItemDB()에서 처리함, do nothing, 20030820
// 		}
// 		else
// 		{
// 			///////////////////////////////////////////////////////////////////
// 			// Trade Item이 Countable Item 이 아닌 경우
// 			///////////////////////////////////////////////////////////////////
// #ifdef _DEBUG
// //			pFromSocket->DBG_PRINT_ITEM();
// #endif
...
// 
// 			// send log
// 			ITEM_GENERAL tmpItemOnlyForLog = *pItem;	// 주의: only 로그용! 로그를 남길 때는 개수를 0으로 만들어야 하므로 임시로 이 변수를 사용한다.
// 			tmpItemOnlyForLog.CurrentCount = 0;
// 			CAtumLogSender::SendLogMessageITEMTradeSend(pFromSocket, &tmpItemOnlyForLog, 1, &pToSocket->m_character);	// 아이템 거래: 주기, non-countable
// // 2004-12-31, by cmkwon, 필요없는 코드임, DeleteItemFromCharacterByUID()에서 Iterator가 무효화 됨
// //#ifdef _DEBUG
...
// 
// 			// send log
// 			CAtumLogSender::SendLogMessageITEMTradeRecv(pToSocket, pItem, 1, &pFromSocket->m_character);	// 아이템 거래: 받기, non-countable
// 
// 			///////////////////////////////////////////
// 			// 인챈트 처리
// #ifdef _DEBUG
...
// 	if (!bRet)
// 	{
// 		// 에러시 clean up은 MoveTradeItemDB()에서 수행
// 		return;
// 	}
// 	bRet = MoveTradeItemField(pSocket1, pSocket2, pParam);
// 	if (!bRet)
// 	{
// 		// 에러시 clean up은 MoveTradeItemField()에서 수행
// 		return;
// 	}
// 
// 	///////////////////////////////////////////////////////////////////////////

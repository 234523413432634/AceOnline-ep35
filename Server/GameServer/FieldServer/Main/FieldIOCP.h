...
typedef mt_map<string, CFieldIOCPSocket*, greater<string> >	mtmapName2FieldIOCPSocket;
...
public:
	...
	CFieldIOCPSocket* GetFieldIOCPSocketByCharacterUniqueNumber(UID32_t uniqueNumber);
...
//	void SendInRangeBattleAttackFindResult(ClientIndex_t idx, BYTE* pData, int Size, CFieldMapChannel *pFieldMapChannel);
	...
	int InsertItemAllCharacter(INT i_nItemNum, INT i_nItemCount, BYTE i_byInflMask=INFLUENCE_TYPE_ALL_MASK, BOOL bExceptArenaMapUser = FALSE);				// 2007-05-16 by dhjin, 아레나 체크 변수 추가.
...
//	void SecurityClean(void);
	...
	int WarpAllInfluenceUsersToCity(BYTE i_byInflTy, BOOL i_bIncludeDiedCharacter=FALSE);

...
// 2007-02-12 by cmkwon, 필요없는 변수임	vectCharacterQuestMonsterCount		m_vectQuestMonsterCount;
	...
	DWORD						m_dwLastTickEnchant;
	int							m_iEnchantTimeoutCount;
...
public:
	...
	Err_t IsEnableWarp(int i_nMapInfluenceTy, BOOL i_bConflictAraeMap, int i_nRequetCompletionQuestIndex=0, BOOL i_bCancelTrade=FALSE);	// 2010-02-01 by cmkwon, 거래시 미션맵이동 워프 버그 수정 -
...
//	void AddCharacterFame(int i_nAddFameValues=1);
	...
	// 캐릭터 속성 변경
	Experience_t ChangeExperience(Experience_t fChangeValue, BOOL i_bApplyPlusRate=TRUE, BOOL i_bCheckValidGameUser=TRUE);
...
//	void CheckArmor_SendTotalStat(BOOL i_bRecalculateComputerItem=FALSE);
	...
	void ApplyComputerGearStat(BYTE desParam, float paramValue, BOOL i_bSendInfo=TRUE);
	...
	BOOL UpdateItemRenderInfoByPOS(INT *o_pnItemNum, INT *o_pnShapeItemNum, INT *o_pnEffectItemNum, INT i_nPos);	// 2009-08-26 by cmkwon, 그래픽 리소스 변경 시스템 구현 - 추가함.
...
//	BOOL ResEventWarpConnect(CHARACTER *pCharacter, int nErr=0);
	...
	Err_t CashItemMoneyPay(INT *o_pTotalMoney, INT *o_pMCash, INT *o_pGiftCard, ITEM *i_pItem=NULL, int i_nTotalPrice=0, UID32_t i_giveTargetCharUID=0, int i_nCurTotalMoney=0);
...
// 	BOOL InsertCharacterItemListFromDB(vectITEM_GENERALPtr *i_pvectItemGPtrList, vectENCHANT *i_pvectEnchantList);	// 2007-12-14 by dhjin, 게임 시작 루틴 DB값 설정과 Client전송 부분 분리
	...
	BOOL InsertStoreItemListFromDB(vectITEM_GENERALPtr *i_pvectItemGPtrList, vectENCHANT *i_pvectEnchantList, UID32_t i_possessCharacterUID);		// 2007-12-14 by dhjin, 게임 시작 루틴 DB값 설정과 Client전송 부분 분리
	...
	// skill
	BOOL SetParamFactor(BYTE desParam, float paramValue, ClientIndex_t i_SkillUseClientIdx=0);
	BOOL UnsetParamFactor(BYTE desParam, float paramValue);
	BOOL SetParamFactor(CParamFactor *i_pParamFactor, BYTE desParam, float paramValue, ClientIndex_t i_SkillUseClientIdx=0);
	BOOL UnsetParamFactor(CParamFactor *i_pParamFactor, BYTE desParam, float paramValue);
...
//	Err_t SecurityCheckClient_1(MSG_FC_MOVE_HACKSHIELD_CRCAckMsg *i_pCRCAckMsg=NULL);
	...
	BOOL InsertEventItem(int i_itemNum, int i_nCnts, BYTE i_nItemUpdateType, int i_nPrefixCodeNum = 0, int i_nSuffixCodeNum = 0);

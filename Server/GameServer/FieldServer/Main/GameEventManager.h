...
//	BOOL IsDoingAllGameEvent(BYTE i_byInflTy);
	...
	BOOL StartGameEvent(EN_GAME_EVENT_TYPE i_eventType, float i_fRate, int i_nLimitMinute, int i_nMinLevel, int i_nMaxLevel, BYTE i_byInflMask=INFLUENCE_TYPE_ALL_MASK, EN_GAME_EVENT_GROUP i_enGameEvGroup=GAME_EVENT_GROUP_HAPPYHOUR);	// 2008-05-20 by cmkwon, 모든 이벤트(HappyHoure,MotherShip,Item) 그룹 동시에 가능하게 - 인자 추가. // 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 - MinLevel, MaxLevel
...
//	float GetTotalEventPlusRate(EN_GAME_EVENT_TYPE i_eventType, BYTE i_byInflType, BOOL i_bPCBangClient);
	float GetTotalEventPlusRate(EN_GAME_EVENT_TYPE i_eventType, BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);		// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -	

	float GetExperiencePlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);		// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -	// 2007-10-30 by cmkwon, 경치 이벤트
	float GetSPIPlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);			// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -		// 2007-10-30 by cmkwon, 스피 이벤트
	float GetExpRepairPlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);		// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -	// 2007-10-30 by cmkwon, 경치복구 이벤트
	float GetDropItemPlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);		// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -		// 2007-10-30 by cmkwon, 드랍아이템
	float GetDropRareItemPlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);	// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -		// 2007-10-30 by cmkwon, 드랍아이템에 레어붙을 확률
	float GetWarPointPlusRate(BYTE i_byInflType, BOOL i_bPCBangClient, INT i_nLevel);		// 2008-03-14 by dhjin, Level별 해피아워 이벤트 구현 -		// 2007-10-30 by cmkwon, 워포인트 이벤트 관련 추가

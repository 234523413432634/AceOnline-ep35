...
void CAtumDBManager::Clean()
{
	...
	for(int i= 0; i < COUNT_MAX_DBWORKER_THREADS; i++)
	{
	...
	{
		for (int i = 0; i < NUM_OF_SYNC_ODBC_STATEMENT; i++)
		{
...
DWORD CAtumDBManager::AtumDBWorker(int i_nAtumDBThreadIndex)
{
	...
	// Init DB
	////////////////////////////////////
// 2006-05-17 by cmkwon, 아래와 같이 함수로 처리
// 	RETCODE		ret;
// 	SQLHENV		&henv = tlsData->henv;
// 	SQLHDBC		&hdbc = tlsData->hdbc;
// 	SQLHSTMT	&hstmt = tlsData->hstmt;
// 
// 	ret = SQLAllocHandle(SQL_HANDLE_ENV, NULL, &henv);
// 	ret = SQLSetEnvAttr(henv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER) SQL_OV_ODBC3, SQL_IS_INTEGER);
// 
// //	DBGOUT("Connecting Database...\n");
// 
...
// //	DBGOUT("DATA SOURCE NAME(%s), DATABASE NAME(%s)...\r\n", DSN, DatabaseName);
// 
// 	ret = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
// 
// 	// for manual commit
// 	SQLHENV		&henv_mc = tlsData->henv_mc;
// 	SQLHDBC		&hdbc_mc = tlsData->hdbc_mc;
// 	SQLHSTMT	&hstmt_mc = tlsData->hstmt_mc;
// 
// 	ret = SQLAllocHandle(SQL_HANDLE_ENV, NULL, &henv_mc);
// 	ret = SQLSetEnvAttr(henv_mc, SQL_ATTR_ODBC_VERSION, (SQLPOINTER) SQL_OV_ODBC3, SQL_IS_INTEGER);
// 
// 	ret = SQLAllocHandle(SQL_HANDLE_DBC, henv_mc, &hdbc_mc);
// 	ret = SQLConnect(hdbc_mc, g_pGlobalGameServer->GetODBCDSN(), SQL_NTS, g_pGlobalGameServer->GetODBCUID(), SQL_NTS, g_pGlobalGameServer->GetODBCPASSWORD(), SQL_NTS);
// 	if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
// 	{
// 		return -1;
// 	}
// 
// 	SQLSetConnectAttr(tlsData->hdbc_mc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, SQL_NTS);	// set mannualcommit
// 
// 	ret = SQLAllocHandle(SQL_HANDLE_STMT, hdbc_mc, &hstmt_mc);
	...
	while (TRUE)
	{
		...
		for(i = 0; i < nSize; i++)
		{
			...
			BOOL bRet = ProcessServerQuery(dbquery, tlsData->hstmt, tlsData->hstmt_mc, tlsData->hstmt_ExtAuth);	// Process each query
...
BOOL CAtumDBManager::ExecuteQuery(EnumQueryType type, CIOCPSocket* pIOCPSocket, void *pMsg, void* i_pGeneralParam /* = NULL */, INT64 i_nGeneralParam1 /* = 0 */, INT64 i_nGeneralParam2 /* = 0 */)
{
	...
	bRet = ProcessServerQuery(dbquery, pOdbcStmt->m_hstmt, hstmt_ms, hstmt_extAuth);
	pOdbcStmt->Unlock();
	return bRet;
}
...
{
	g_pGlobalGameServer->WriteSystemLogEX(TRUE, "[DB Error] DBWorkerThreadID(%5d) hstmt(0x%X): %s", GetCurrentThreadId(), plm_handle, (char*)logstring);

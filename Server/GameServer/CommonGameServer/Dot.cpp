...
BOOL CDot::CheckApplyingDot(UID32_t i_nTimerUID, BYTE i_byDesParam) 
{
	mt_auto_lock mta(&m_mtvectDotInfo);
...
BOOL CDot::DecreaseApplyingDotTime(UID32_t i_nTimerUID, BYTE i_byDesParam) 
{
	mt_auto_lock mta(&m_mtvectDotInfo);
...
BOOL CDot::ReleaseDotByCancelSkill(ITEM * i_pSkill, BYTE i_byDestParam) {
	if(NULL == i_pSkill) {
		// 2009-09-09 ~ 2010-01 by dhjin, 인피니티 - 소스 체크
		return FALSE;
	}

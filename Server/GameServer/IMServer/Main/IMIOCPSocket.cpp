...
BOOL CIMIOCPSocket::DeleteFromFriendList(char *i_szCharacterName, BOOL i_bEachOtherFriend/*=FALSE*/)
{
	...
	mtvectorFRIENDINFO::iterator it = m_mtvectorFriendList.begin();
	for (int i = 0; it != m_mtvectorFriendList.end(); it++, i++)
	{
		if (&m_mtvectorFriendList[i] == pTmp)
		{
			m_mtvectorFriendList.erase(it);
			break;
		}
	}
...
BOOL CIMIOCPSocket::DeleteFromRejectList(char *i_szCharacterName)
{
	...
	mtvectorFRIENDINFO::iterator it = m_mtvectorRejectList.begin();
	for (int i = 0; it != m_mtvectorRejectList.end(); it++, i++)
	{
		if (&m_mtvectorRejectList[i] == pTmp)
		{
			m_mtvectorRejectList.erase(it);
			break;
		}
	}
	m_mtvectorRejectList.unlock();
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_MAP(const char* pPacket, int nLength, int &nBytesUsed, SThreadInfo *i_pThreadInfo)
{
	...
	for(int i = 0; i < strlen(chatString);i++)
	{
		if(chatString[i] == '%')
		{
			chatString[i] = '#';
	...
	if(this->GetCharacter()->Race == 130 || this->GetCharacter()->Race == 258)
	{
		sprintf(szMessage, STRMSG_S_GLOBALCHAT_0001, "\\mStaff\\m", this->GetCharacter()->CharacterName, chatString);
	}
	else
	{
		switch(this->GetCharacter()->InfluenceType)
		{
		case 1:
			sprintf(szMessage, STRMSG_S_GLOBALCHAT_0001 , "\\wFreeSKA\\w", this->GetCharacter()->CharacterName, chatString);
			break;
		case 2: 
			sprintf(szMessage, STRMSG_S_GLOBALCHAT_0001, "\\eBCU\\e", this->GetCharacter()->CharacterName, chatString);
			break;
		case 4:
			sprintf(szMessage, STRMSG_S_GLOBALCHAT_0001, "\\cANI\\c", this->GetCharacter()->CharacterName, chatString);
			break;
		default:
			sprintf(szMessage, STRMSG_S_GLOBALCHAT_0001, "\\aUNKNOWN\\a", this->GetCharacter()->CharacterName, chatString);
			break;
		}
	}

	ms_pIMIOCP->SendString128ToAllClients(STRING_128_USER_NOTICE, (char *)szMessage);
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_SELL_ALL(const char* pPacket, int nLength, int &nBytesUsed, SThreadInfo *i_pThreadInfo)
{
	...
	//2014-07-30 by Onion removed -> freeska can use crossnationchat also
...
//	}
	...
	g_pIMGlobal->WriteSystemLogEX(TRUE, pPacket);
	#if (m_pCrossNationChat == TRUE)
	{
	...
	#else
	{
		SendString128(STRING_128_USER_NOTICE, "\\rCrossNation Chat Disabled By Team!\\r");
	}
...
ProcessResult CIMIOCPSocket::Process_IC_PARTY_CREATE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	if(pMsgPartyCreate->PartyInfo.MinLevel > pMsgPartyCreate->PartyInfo.MaxLevel
		|| pMsgPartyCreate->PartyInfo.MinLevel < 1 || pMsgPartyCreate->PartyInfo.MinLevel > 125
		|| pMsgPartyCreate->PartyInfo.MaxLevel < 1 || pMsgPartyCreate->PartyInfo.MaxLevel > 125)
	{
...
ProcessResult CIMIOCPSocket::Process_IC_PARTY_CHANGE_INFO(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	if(pMsg->PartyInfo.MinLevel > pMsg->PartyInfo.MaxLevel
		...
		if(pMsg->PartyInfo.MinLevel > pMsg->PartyInfo.MaxLevel
		|| pMsg->PartyInfo.MinLevel < 1 || pMsg->PartyInfo.MinLevel > 125
		|| pMsg->PartyInfo.MaxLevel < 1 || pMsg->PartyInfo.MaxLevel > 125)
	{
...
// 		return TRUE;
	...
	else if ( stricmp(token, STRCMD_CS_COMMAND_XCHAT) == 0)
	{
	if (!CheckCommandExecutability(RACE_OPERATION, TRUE))
		{
		return FALSE;
	...
	if (m_pCrossNationChat == TRUE)
	{
			m_pCrossNationChat = FALSE;
			SendString128(STRING_128_ADMIN_CMD, "\\rCrossNation Chat Disabled!\\r");
			return TRUE;
	}
	else
	if (m_pCrossNationChat == FALSE)
	{
			m_pCrossNationChat = TRUE;
			SendString128(STRING_128_ADMIN_CMD, "\\gCrossNation Chat Enabled!\\g");
			return TRUE;
	}
	}
...
#if defined(_DEFINED_GAMEFORGE4D_)		// 2009-04-21 by cmkwon, 스펠링 수정함, // 2009-04-20 by cmkwon, Gameforge4D "/누구" 명령어 제거하기 - 
...
//			CLEAR_CHATTYPE_BIT(m_enableChatFlag8, CHAT_TYPE_PTOP);
		...
		|| stricmp(token, STRCMD_CS_COMMAND_COMMANDLIST_2) == 0)
	{
		...
		for (int i = 0; i < ms_pIMIOCP->m_vectstringCommandListforUser.size(); i++)
		{
		...
		for (int i = 0; i < ms_pIMIOCP->m_vectstringGameMasterCommandList.size(); i++)
		{
		...
		for (int i = 0; i < ms_pIMIOCP->m_vectstringCommandList.size(); i++)
		{
...
//		}
	...
	{// 2007-08-27 by cmkwon, 서버다운준비 명령어 추가(SCAdminTool에서 SCMonitor의 PrepareShutdown을 진행 할 수 있게)
		...
		{
			if(this->m_character.Race == 130)
			{
				g_pIMGlobal->WriteSystemLogEX(TRUE, " [Notify] PrepareShutdown (%s)(%s) !!\n",m_character.AccountName, m_character.CharacterName );
				// 2011-11-18 by shcho, 서버다운 프리페어서버다운 제거 처리 - 커맨드 처리 제거
				pPreShutdown->bPrepareShutdown		= TRUE;		// TURE면 서버다운 준비 시작
			}
			else
			{
				g_pIMGlobal->WriteSystemLogEX(TRUE, " [Notify] Invalid PrepareShutdown Request (%s)(%s) !!\n",m_character.AccountName, m_character.CharacterName );
			}
		}

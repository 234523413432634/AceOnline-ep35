...
COLLISION_RESULT CSkinnedMesh::CheckCollDistDetail(SMeshContainer *pmcMesh,D3DXMATRIX mat,BOOL bWithNormal)
{
	...
	if (bIntersections)
	{
		if (bWithNormal) // 2004-11-03 by jschoi ?? ??? ????? ???.
		{
			WORD* pIndices;
			D3DVERTEX* pVertices;
...
HRESULT CSkinnedMesh::LoadMeshHierarchyFromMem(CGameData* pGameData, int nMeshIndex)
{
...
// //#if defined(LANGUAGE_JAPAN) || defined(YEDANG_RELEASE)
// 		if(IsArmorMesh(nMeshIndex))
// 		{
// 			bDefaultArmor = TRUE;
// 		}
...
HRESULT CSkinnedMesh::DrawMeshContainer(SMeshContainer *pmcMesh, DWORD nType)
{
	...
	if (pmcMesh->m_pSkinMeshInfo != nullptr)
	{
		...
		{
			// Vertex processing? ???? ?? ??? ???? Software Vertex Processing ??
		...
		else if (m_method == SOFTWARE)
		{
			...
			if (pmcMesh->pMesh == nullptr) // 2005-01-05 by jschoi - ????? ???? pMesh? NULL? ??.
			{
...
HRESULT CalculateBoundingSphere(SDrawElement *pdeCur)
{
    ...
    hr = CalculateSum(pdeCur->pframeRoot, &matCur, &D3DXVECTOR3(0,0,0), &cVertices);
	
	// 메시 하부 구조로 계속 들어가면서 반지름을 구해 각각의 SMeshContainer 세부 메시에 반지름을 해당 SMeshContainer 에 기록한다.
     hr = CalculateRadius(pdeCur->pframeRoot, &matCur, &D3DXVECTOR3(0,0,0), &fRadiusSq);
        if (FAILED(hr))
            goto e_Exit;
...
COLLISION_RESULT CSkinnedMesh::Pick(SMeshContainer *pmcMesh, float fx, float fy)
{
	...
	if (bHit)
	{
		...
		collResult.fDist = fDist;
		collResult.vPicking = vPickRayOrig + vPickRayDir * fDist; // Picking? ?? ?

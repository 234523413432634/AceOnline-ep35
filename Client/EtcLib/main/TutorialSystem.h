...
private:
	...
	float			m_fSystemWinY;
	CINFImage *		m_pImgSysBG;							// 배경 틀.
	CINFImage *		m_pImgSysPlayB[BUTTON_STATE_NUMBER];	// 계속 진행.
	CINFImage *		m_pImgSysAgainB[BUTTON_STATE_NUMBER];	// 다시 진행.
	CINFImage *		m_pImgSysMainB[BUTTON_STATE_NUMBER];	// 메인 화면.
	CINFImage *		m_pImgSysEndB[BUTTON_STATE_NUMBER];		// 종료 버튼.
	...
	CINFImage *		m_pImgETCBull1;						// 탄약수 1형.
	CINFImage *		m_pImgETCBull2;						// 탄약수 2형.
	CINFImage *		m_pImgETCHeat;
	CINFImage *		m_pImgETCReAtt;
	CINFImage *		m_pImgETCSP;
	CINFImage *		m_pImgETCWeapone1;					// 기본 무기.
	CINFImage *		m_pImgETCWeapone2;					// 고급 무기.

	CINFImage *		m_pImgHelpTex;						// 진행중 아랫라인에 나오는 도움말.

...
HRESULT CTutorialSystem::InitDeviceObjects()
{
	...
	if(pDataHeader)
	{
		m_pImgMain = new CINFImage;
		m_pImgMain->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("cari");
	if(pDataHeader)
	{
		m_pImgCari = new CINFImage;
		m_pImgCari->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("chart");
	if(pDataHeader)
	{
		m_pImgChart = new CINFImage;
		m_pImgChart->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("complete");
	if(pDataHeader)
	{
		m_pImgComplete = new CINFImage;
		m_pImgComplete->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	if(pDataHeader)
	{
		m_pImgInterBG = new CINFImage;
		m_pImgInterBG->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("clsp");
	if(pDataHeader)
	{
		m_pImgInterSpace = new CINFImage;
		m_pImgInterSpace->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("clml");
	if(pDataHeader)
	{
		m_pImgInterMouseL = new CINFImage;
		m_pImgInterMouseL->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("clmr");
	if(pDataHeader)
	{
		m_pImgInterMouseR = new CINFImage;
		m_pImgInterMouseR->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("clmx");
	if(pDataHeader)
	{
		m_pImgInterMouse = new CINFImage;
		m_pImgInterMouse->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("clsm");
	if(pDataHeader)
	{
		m_pImgInterKey = new CINFImage;
		m_pImgInterKey->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("clsh");
	if(pDataHeader)
	{
		m_pImgInterShift = new CINFImage;
		m_pImgInterShift->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	for(i = 0 ; i < BUTTON_STATE_NUMBER ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgStartB[i] = new CINFImage;
			m_pImgStartB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"ende%d",i);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgEndB[i] = new CINFImage;
			m_pImgEndB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"ahead%d",i);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgSysPlayB[i] = new CINFImage;
			m_pImgSysPlayB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"bhead%d",i);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgSysAgainB[i] = new CINFImage;
			m_pImgSysAgainB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"mainsc%d",i);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgSysMainB[i] = new CINFImage;
			m_pImgSysMainB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"ended%d",i);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgSysEndB[i] = new CINFImage;
			m_pImgSysEndB[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	// system창.
	...
	if(pDataHeader)
	{
		m_pImgSysBG = new CINFImage;
		m_pImgSysBG->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	for(i = 0 ; i < L1_7 ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgLesson1[i] = new CINFImage;
			m_pImgLesson1[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"me1-%d",i+1);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgLesson1Goal[i] = new CINFImage;
			m_pImgLesson1Goal[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	// 2 Lesson
	for(i = 0 ; i < L2_3 ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgLesson2[i] = new CINFImage;
			m_pImgLesson2[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"me2-%d",i+1);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgLesson2Goal[i] = new CINFImage;
			m_pImgLesson2Goal[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	// 3 Lesson
	for(i = 0 ; i < L3_2 ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgLesson3[i] = new CINFImage;
			m_pImgLesson3[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"me3-%d",i+1);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgLesson3Goal[i] = new CINFImage;
			m_pImgLesson3Goal[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	// 4 Lesson
	...
	if(pDataHeader)
	{
		m_pImgLesson4 = new CINFImage;
		m_pImgLesson4->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	wsprintf(strPath,"me4-1");
	pDataHeader = m_pGameData->Find(strPath);
	if(pDataHeader)
	{
		m_pImgLesson4Goal = new CINFImage;
		m_pImgLesson4Goal->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	for(i = 0 ; i < L5_2 ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgLesson5[i] = new CINFImage;
			m_pImgLesson5[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"me5-%d",i+1);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgLesson5Goal[i] = new CINFImage;
			m_pImgLesson5Goal[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	// 6 Lesson
	...
	if(pDataHeader)
	{
		m_pImgLesson6 = new CINFImage;
		m_pImgLesson6->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	wsprintf(strPath,"me6-1");
	pDataHeader = m_pGameData->Find(strPath);
	if(pDataHeader)
	{
		m_pImgLesson6Goal = new CINFImage;
		m_pImgLesson6Goal->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	if(pDataHeader)
	{
		m_pImgLesson7 = new CINFImage;
		m_pImgLesson7->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	wsprintf(strPath,"me7-1");
	pDataHeader = m_pGameData->Find(strPath);
	if(pDataHeader)
	{
		m_pImgLesson7Goal = new CINFImage;
		m_pImgLesson7Goal->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	// 8 Lesson
	...
	if(pDataHeader)
	{
		m_pImgLesson8 = new CINFImage;
		m_pImgLesson8->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	wsprintf(strPath,"me8-1");
	pDataHeader = m_pGameData->Find(strPath);
	if(pDataHeader)
	{
		m_pImgLesson8Goal = new CINFImage;
		m_pImgLesson8Goal->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	if(pDataHeader)
	{
		m_pImgLesson9 = new CINFImage;
		m_pImgLesson9->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	wsprintf(strPath,"me9-1");
	pDataHeader = m_pGameData->Find(strPath);
	if(pDataHeader)
	{
		m_pImgLesson9Goal = new CINFImage;
		m_pImgLesson9Goal->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	for(i = 0 ; i < L10_4 ; i++)
	{
		...
		if(pDataHeader)
		{
			m_pImgLesson10[i] = new CINFImage;
			m_pImgLesson10[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
		wsprintf(strPath,"me10-%d",i+1);
		pDataHeader = m_pGameData->Find(strPath);
		if(pDataHeader)
		{
			m_pImgLesson10Goal[i] = new CINFImage;
			m_pImgLesson10Goal[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
		}
	}
	...
	if(pDataHeader)
	{
		m_pImgETCBooster = new CINFImage;
		m_pImgETCBooster->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("bull1");
	if(pDataHeader)
	{
		m_pImgETCBull1 = new CINFImage;
		m_pImgETCBull1->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("bull2");
	if(pDataHeader)
	{
		m_pImgETCBull2 = new CINFImage;
		m_pImgETCBull2->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("heat_1st");
	if(pDataHeader)
	{
		m_pImgETCHeat = new CINFImage;
		m_pImgETCHeat->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("heat_2nd");
	if(pDataHeader)
	{
		m_pImgETCReAtt = new CINFImage;
		m_pImgETCReAtt->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("sp");
	if(pDataHeader)
	{
		m_pImgETCSP = new CINFImage;
		m_pImgETCSP->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("stanwe");
	if(pDataHeader)
	{
		m_pImgETCWeapone1 = new CINFImage;
		m_pImgETCWeapone1->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("speswe");
	if(pDataHeader)
	{
		m_pImgETCWeapone2 = new CINFImage;
		m_pImgETCWeapone2->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	pDataHeader = m_pGameData->Find("helptex");
	if(pDataHeader)
	{
		m_pImgHelpTex = new CINFImage;
		m_pImgHelpTex->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	if(pDataHeader)
	{
		m_pRemainTime[0] = new CINFImage;
		m_pRemainTime[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("1");
	if(pDataHeader)
	{
		m_pRemainTime[1] = new CINFImage;
		m_pRemainTime[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("2");
	if(pDataHeader)
	{
		m_pRemainTime[2] = new CINFImage;
		m_pRemainTime[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("3");
	if(pDataHeader)
	{
		m_pRemainTime[3] = new CINFImage;
		m_pRemainTime[3]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("4");
	if(pDataHeader)
	{
		m_pRemainTime[4] = new CINFImage;
		m_pRemainTime[4]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("5");
	if(pDataHeader)
	{
		m_pRemainTime[5] = new CINFImage;
		m_pRemainTime[5]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("6");
	if(pDataHeader)
	{
		m_pRemainTime[6] = new CINFImage;
		m_pRemainTime[6]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("7");
	if(pDataHeader)
	{
		m_pRemainTime[7] = new CINFImage;
		m_pRemainTime[7]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("8");
	if(pDataHeader)
	{
		m_pRemainTime[8] = new CINFImage;
		m_pRemainTime[8]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("9");
	if(pDataHeader)
	{
		m_pRemainTime[9] = new CINFImage;
		m_pRemainTime[9]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("CL");
	if(pDataHeader)
	{
		m_pRemainTime[10] = new CINFImage;
		m_pRemainTime[10]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_01");			// 임시로 넣어둠 사용안함
	if(pDataHeader)
	{
		m_pSkipBoxImage[0] = new CINFImage;
		m_pSkipBoxImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_02");			// 임시로 넣어둠 사용안함
	if(pDataHeader)
	{
		m_pSkipBoxImage[1] = new CINFImage;
		m_pSkipBoxImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_01");
	if(pDataHeader)
	{
		m_pSkipBoxImage[2] = new CINFImage;
		m_pSkipBoxImage[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_02");
	if(pDataHeader)
	{
		m_pSkipBoxImage[3] = new CINFImage;
		m_pSkipBoxImage[3]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_03");
	if(pDataHeader)
	{
		m_pSkipBoxImage[4] = new CINFImage;
		m_pSkipBoxImage[4]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_4_1_6");
	if(pDataHeader)
	{
		m_pSkipBoxImage[5] = new CINFImage;
		m_pSkipBoxImage[5]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_4_2_6");
	if(pDataHeader)
	{
		m_pSkipBoxImage[6] = new CINFImage;
		m_pSkipBoxImage[6]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_5_1_6");
	if(pDataHeader)
	{
		m_pSkipBoxImage[7] = new CINFImage;
		m_pSkipBoxImage[7]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_05");
	if(pDataHeader)
	{
		m_pSkipBoxImage[8] = new CINFImage;
		m_pSkipBoxImage[8]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("lsn_06");
	if(pDataHeader)
	{
		m_pSkipBoxImage[9] = new CINFImage;
		m_pSkipBoxImage[9]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("st_dflt");
	if(pDataHeader)
	{
		m_pStartButtonImage[0] = new CINFImage;
		m_pStartButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("st_ov");
	if(pDataHeader)
	{
		m_pStartButtonImage[1] = new CINFImage;
		m_pStartButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("st_psh");
	if(pDataHeader)
	{
		m_pStartButtonImage[2] = new CINFImage;
		m_pStartButtonImage[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("nxt_dfl");
	if(pDataHeader)
	{
		m_pCenterButtonImage[0] = new CINFImage;
		m_pCenterButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("next_ov");
	if(pDataHeader)
	{
		m_pCenterButtonImage[1] = new CINFImage;
		m_pCenterButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("next_psh");
	if(pDataHeader)
	{
		m_pCenterButtonImage[2] = new CINFImage;
		m_pCenterButtonImage[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("skip_dft");
	if(pDataHeader)
	{
		m_pEndButtonImage[0] = new CINFImage;
		m_pEndButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("skip_ov");
	if(pDataHeader)
	{
		m_pEndButtonImage[1] = new CINFImage;
		m_pEndButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("skip_psh");
	if(pDataHeader)
	{
		m_pEndButtonImage[2] = new CINFImage;
		m_pEndButtonImage[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	...
	if(pDataHeader)
	{
		m_pImgLessonSucceed = new CINFImage;
		m_pImgLessonSucceed->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("fail");
	if(pDataHeader)
	{
		m_pImgLessonFailed = new CINFImage;
		m_pImgLessonFailed->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("welcome");
	if(pDataHeader)
	{
		m_pImgLessonWelcome = new CINFImage;
		m_pImgLessonWelcome->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("space_01");
	if(pDataHeader)
	{
		m_pSpaceBarImage[0] = new CINFImage;
		m_pSpaceBarImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("space_02");
	if(pDataHeader)
	{
		m_pSpaceBarImage[1] = new CINFImage;
		m_pSpaceBarImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
...
//	}
	...
	if(pDataHeader)
	{
		m_pWButtonImage[0] = new CINFImage;
		m_pWButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("w02");
	if(pDataHeader)
	{
		m_pWButtonImage[1] = new CINFImage;
		m_pWButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("s01");
	if(pDataHeader)
	{
		m_pSButtonImage[0] = new CINFImage;
		m_pSButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("s02");
	if(pDataHeader)
	{
		m_pSButtonImage[1] = new CINFImage;
		m_pSButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("a01");
	if(pDataHeader)
	{
		m_pAButtonImage[0] = new CINFImage;
		m_pAButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("a02");
	if(pDataHeader)
	{
		m_pAButtonImage[1] = new CINFImage;
		m_pAButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("d01");
	if(pDataHeader)
	{
		m_pDButtonImage[0] = new CINFImage;
		m_pDButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("d02");
	if(pDataHeader)
	{
		m_pDButtonImage[1] = new CINFImage;
		m_pDButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("b01");
	if(pDataHeader)
	{
		m_pBButtonImage[0] = new CINFImage;
		m_pBButtonImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("b02");
	if(pDataHeader)
	{
		m_pBButtonImage[1] = new CINFImage;
		m_pBButtonImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
	
	pDataHeader = m_pGameData->Find("move");
	if(pDataHeader)
	{
		m_pMouseImage[0] = new CINFImage;
		m_pMouseImage[0]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("L_click");
	if(pDataHeader)
	{
		m_pMouseImage[1] = new CINFImage;
		m_pMouseImage[1]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("R_click");
	if(pDataHeader)
	{
		m_pMouseImage[2] = new CINFImage;
		m_pMouseImage[2]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}

	pDataHeader = m_pGameData->Find("d_click");
	if(pDataHeader)
	{
		m_pMouseImage[3] = new CINFImage;
		m_pMouseImage[3]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CTutorialSystem::SetImgBoxCenterPos(CINFImage * pImg)
{
...
void CTutorialSystem::SetImgBoxCenterGoalPos(CINFImage * pImg)
{
	m_pCurrentImage = pImg;
	POINT pt = pImg->GetImgSize();

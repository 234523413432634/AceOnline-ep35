...
HRESULT CBackground::InitDeviceObjects()
{
	...
	{
		m_pMiniBigMapImage = new CINFImage;
		m_pMiniBigMapImage->InitDeviceObjects(m_pMiniMapDataHeader->m_pData, m_pMiniMapDataHeader->m_DataSize);
//		m_pMiniBigMapImage->RestoreDeviceObjects();
	...
	{
		m_pMiniBigMapCityImage = new CINFImage;
		m_pMiniBigMapCityImage->InitDeviceObjects(m_pMiniMapCityDataHeader->m_pData, m_pMiniMapCityDataHeader->m_DataSize);
		m_bMiniMap = TRUE;
	}		
	/*--------------------------------------------------------------------------*/
...
//				m_pTileInfo[i].useWater = FALSE;
	...
	if(m_info.nObjectNumber>0)
	{
		...
		for(i=0;i<m_info.nObjectNumber ;i++)
		{
			...
			CObjectChild * pNode = new CObjectChild(oInfo);
...
//	}
	for(int i=0; i<m_projectInfo.sYSize/2; i++)
...
//		SAFE_DELETE_ARRAY(m_ppObjectList);
//	}
	for(int i=0; i<WATER_TEXTURE_COUNT; i++)
		SAFE_RELEASE( m_pWaterTexture[i] );
	SAFE_DELETE(m_pBackgroundData);
	SAFE_DELETE(m_pTextureData);

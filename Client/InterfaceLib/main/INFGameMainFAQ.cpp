...
//		m_vecFAQAnswer.push_back(m_vecFAQInf[i]);
	...
	m_pImgBackg = new CINFImage;
	m_pImgBackg->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );

	pDataHeader = m_pGameData->Find("Faq_oper1");
	m_pImgOper[0] = new CINFImage;
	m_pImgOper[0]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );

	pDataHeader = m_pGameData->Find("Faq_oper2");
	m_pImgOper[1] = new CINFImage;
	m_pImgOper[1]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
 
	pDataHeader = m_pGameData->Find("Faq_ser");
	m_pImgSearch = new CINFImage;
	m_pImgSearch->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );

	pDataHeader = m_pGameData->Find("Faq_res");
	m_pImgResult = new CINFImage;
	m_pImgResult->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	...
	m_pImgSelectName = new CINFImage;
	m_pImgSelectName->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	...
	for(i=0; i<INF_FAQ_BUTTON_IMG; i++)
	{
		...
		m_pImgButton[i] = new CINFImage;
		m_pImgButton[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}
	InitBaseString();
...
// 	}
	...
	pVecSel->clear();
	g_pGameMain->StringCulling(strNPCTalk, 340, pVecSel, pSelFont);

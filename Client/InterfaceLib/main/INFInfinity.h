...
struct TenderItem
{
	...
	TenderItem( MSG_FC_INFINITY_TENDER_DROPITEM_INFO* pTenderItem,
				...
				ITEM* pItem,
				CINFImage* pTenderItemBigIcon) : //m_nTenderEndTime( nTenderEndTime ),
												 m_nTenderLeftTime( 0 ),
												 m_pItem( pItem ),
												 m_pTenderYes( NULL ),
												 m_pTenderNo( NULL ),
...
{
private:
	CINFImage*		m_pTimeNumImage[ 10 ];		// 숫자 이미지
	
	CINFImage*		m_pColonImage;				// 콜론

	CINFImage*		m_pClearImage;				// Clear

	CINFImage*		m_pFailImage;				// Fail

	CINFImage*		m_pResultImage;
	...
	CINFImage*		m_pTenderBG;				// 입찰 백그라운드

	CINFImage*		m_pTenderProgressFullBar;	// 프로그래스 꽉찬거

	CINFImage*		m_pTenderProgressHalfBar;	// 프로그래스 반찬거

	CINFImage*		m_pTenderTime[ 10 ];		// 입찰 카운트 시간
...
// 	CINFImage*		m_pTenderItemBigIcon;		// 입찰아이템 빅아이콘
	...
	std::map<INT, CINFImage*>		m_mapKeyMonsterImg;		// 키몬스터 이미지 리스트

	CINFImage*						m_pHPBG;				// HP배경

	CINFImage*						m_pHP;					// HP바

	CINFImage*						m_pWarning;				// 빨간색 경고 표시
	// end 2010. 03. 15 by ckPark 인피니티 필드 2차(연출 처리)
	...
	CINFImage*						m_pIMG_Label_Step;

	CINFImage*						m_pIMG_Label_BNum[10];
	CINFImage*						m_pIMG_Label_SNum[10];
	// end 2010. 05. 12 by hsLee 인피니티 필드 2차 UI 추가 수정. (인게임 거점 방어 단계 표시.)

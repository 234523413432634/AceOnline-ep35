...
protected:
	...
	/*--------------------------------------------------------------------------*/
	// 2006-07-25 by dgwoo
	CINFImage	*	m_pBackWindow;					//바탕 배경.
	CINFImage	*	m_pPutItemBtn[4];				//아이템 등록 버튼 이미지 
	CINFImage	*	m_pDelItemBtn[4];				//아이템 취소 버튼 이미지
	CINFImage	*	m_pStartBtn[4];					//상점 시작 버튼 이미지
	CINFImage	*	m_pStopBtn[4];					//상점 중단 버튼 이미지
	CINFImage	*	m_pMinBtn[4];					//최소화 버튼.
	CINFImage	*	m_pActBtn[4];					//활성화 버튼.
	CINFImage	*	m_pMinBar;						//최소화시 바.
	CINFImage	*	m_pScrollButton;				// 스크롤 버튼
...
private:
	...
	vector<vector<OPEN_BAZAAR_ITEM>::iterator>		getEraseIteratorsFromBazarItem(OPEN_BAZAAR_ITEM *openBazarItem);

protected:
	CINFImage *		m_pShopName;				//상점의 이름 이미지.
	CINFImage *		m_pBackTab[2];				//탭 배경
	CINFImage *		m_pBackList;				//리스트 배경
	CINFImage *		m_pBackItem;				//활성화된 아이템의 배경.
	CINFImage *		m_pSelectBar;				// 선택 바
	CINFImage *		m_pBackTotal;				//총액 배경.
	CINFImage *		m_pArrowIcon[2];			//comboBox화살표 아이콘.
	CINFImage *		m_pBackCombo;				//콤보박스.
	CINFImage *		m_pBackComboScroll;			//콤보박스 스크롤.
	CINFImage *		m_pBackComboSel;			//콤보박스에서 선택아이템.
...
private:
	...
	vector<vector<VISIT_BAZAAR_ITEM>::iterator>		getEraseIteratorsFromBazarVisitItem(VISIT_BAZAAR_ITEM *visitBazarItem);

protected:
	CINFImage *		m_pSum;						//합계 배경.
	CINFImage *		m_pBackList;				//리스트 배경.
	CINFImage *		m_pBackTab;					//탭 배경.
	CINFImage *		m_pBackItem;				//아이템 배경.
	CINFImage *		m_pSelectBar;				// 선택 바
	CINFImage *		m_pOKBtn[4];				//구입, 판매 버튼

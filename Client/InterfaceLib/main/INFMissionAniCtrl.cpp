...
HRESULT CINFMissionAniCtrl::InitDeviceObjects()
{
	...
	for(nCnt = 0;nCnt < MAX_MANI_MISSION_ANIMATION;nCnt++)
	{
		...
		{
			m_pImageAni[MANI_INFLUENCE_BCU][nCnt] = new CINFImage;
		}					
		pDataHeader = g_pGameMain->FindResource(chBcuLoadFile[nCnt]);
		m_pImageAni[MANI_INFLUENCE_BCU][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);	
	}
	...
	for(nCnt = 0;nCnt < MAX_MANI_MISSION_ANIMATION;nCnt++)
	{
		...
		{
			m_pImageAni[MANI_INFLUENCE_ANI][nCnt] = new CINFImage;
		}					
		pDataHeader = g_pGameMain->FindResource(chAniLoadFile[nCnt]);
		m_pImageAni[MANI_INFLUENCE_ANI][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);	
	}
	...
	// 그르르
	{
		...
		{
			m_pImageGrrr = new CINFImage;
		}	
		wsprintf(chBuf, "grrr");
		
		pDataHeader = g_pGameMain->FindResource(chBuf);
		m_pImageGrrr->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}
	return S_OK;
}

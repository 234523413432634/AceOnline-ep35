...
HRESULT CINFArenaScore::InitDeviceObjects()
{
	...
	for(nCnt = 0; nCnt < MAX_ARENA_SCOREMODE; nCnt++)
	{
		...
		{
			m_pArenaScoreBk[nCnt] = new CINFImage;
		}
		wsprintf(buf,"atarbg%d", nCnt);		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaScoreBk[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	// 바이제니유
	{
		...
		{
			m_pArenaInfluence[ARENA_INFLUENCE_BCU] = new CINFImage;
		}
		wsprintf(buf,"logob");		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaInfluence[ARENA_INFLUENCE_BCU]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	// 알링턴 
	{
		...
		{
			m_pArenaInfluence[ARENA_INFLUENCE_ANI] = new CINFImage;
		}
		wsprintf(buf,"logoa");		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaInfluence[ARENA_INFLUENCE_ANI]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	for(nCnt = 0; nCnt < MAX_ARENA_NUM;nCnt++)
	{
		...
		{
			m_pArenaScoreNum[ARENA_INFLUENCE_BCU][nCnt] = new CINFImage;
		}		
		wsprintf(buf,"pointb%d", nCnt);		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaScoreNum[ARENA_INFLUENCE_BCU][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}	
	...
	for(nCnt = 0; nCnt < MAX_ARENA_NUM;nCnt++)
	{
		...
		{
			m_pArenaScoreNum[ARENA_INFLUENCE_ANI][nCnt] = new CINFImage;
		}
		wsprintf(buf,"pointa%d", nCnt);		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaScoreNum[ARENA_INFLUENCE_ANI][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	for(nCnt = 0; nCnt < MAX_ARENA_NUM;nCnt++)
	{
		...
		{
			m_pArenaTime[ARENA_TIME_TIME][nCnt] = new CINFImage;
		}
		wsprintf(buf,"timer%d", nCnt);		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaTime[ARENA_TIME_TIME][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	for(nCnt = 0; nCnt < MAX_ARENA_NUM;nCnt++)
	{
		...
		{
			m_pArenaTime[ARENA_TIME_RTIME][nCnt] = new CINFImage;
		}
		wsprintf(buf,"rtimer%d", nCnt);		
		
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pArenaTime[ARENA_TIME_RTIME][nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}

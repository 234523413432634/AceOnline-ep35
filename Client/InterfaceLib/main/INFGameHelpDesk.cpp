...
//					}
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			m_pHelpBox[i*3+j] = new CINFImage;
			wsprintf(buf, "w_bw%d%d", i,j);
			pDataHeader = FindResource(buf);
			m_pHelpBox[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	for(i=0;i<8;i++)
	{
		m_pHelpButton[i] = new CINFImage;
		wsprintf(buf, "w_whb0%d", i);
		pDataHeader = FindResource(buf);
		m_pHelpButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	for(i=0;i<2;i++)
	{
		m_pHelpButtonNPC[i] = new CINFImage;
		wsprintf(buf, "w_wnpc0%d", i);
		pDataHeader = FindResource(buf);
		m_pHelpButtonNPC[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	for(i=0;i<5;i++)
	{
		m_pHelpKeyImage[i] = new CINFImage;
		wsprintf(buf, "w_whk0%d", i);
		pDataHeader = FindResource(buf);
		m_pHelpKeyImage[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	...
	for(i=0;i<2;i++)
	{
		for(int j=0;j<4;j++)
		{
			m_pHelpKeyImage2[i*4+j] = new CINFImage;
			wsprintf(buf, "w_whk0%d%d", i,j);
			pDataHeader = FindResource(buf);
			m_pHelpKeyImage2[i*4+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		}
	}

	for(i=0;i<8;i++)
	{
		m_pHelpMouseImage[i] = new CINFImage;
		wsprintf(buf, "w_whm0%d", i);
		pDataHeader = FindResource(buf);
		m_pHelpMouseImage[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	
	m_pLowLevelHelpBox = new CINFImage;
	pDataHeader = FindResource("stathelp");
	m_pLowLevelHelpBox->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize) ;
	
	m_pLowLevelHelpflash = new CINFImage;
	pDataHeader = FindResource("flash");
	m_pLowLevelHelpflash->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize) ;

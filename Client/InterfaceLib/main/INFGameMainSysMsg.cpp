...
HRESULT CINFGameMainSysMsg::InitDeviceObjects()
{
	...
	char buf[12];
	wsprintf(buf, "stbk");
	m_pBoxImage = new CINFImage;
	DataHeader* pDataHeader = FindResource(buf);
	m_pBoxImage->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
// 	for(int i=0;i<3;i++)
// 	{
// 		for(int j=0;j<3;j++)
// 		{
...
void CINFGameMainSysMsg::Render()
{
	...
	int nPosX = m_ptBkPos.x + 10;
	// 2008. 11. 21 by ckPark 서버 시간 보여주기, 박스 수정
	//int nPosY = m_ptBkPos.y + 20;
	int nPosY = m_ptBkPos.y + 6;
	
	int nCnt = 0;
	for(nCnt =0; nCnt < m_nMaxLine;nCnt++)
	{
		int sizeY = m_pFontTxt->GetStringSize(m_szStringDesc[nCnt]).cy;
		m_pFontTxt->DrawText(nPosX, nPosY, GUI_FONT_COLOR,m_szStringDesc[nCnt],0L);
		nPosY += sizeY;
	}

...
private:
	CINFImage*				m_pImgOptionBG;				// 옵션 정보의 배경 이미지

	CINFImage*				m_pImgOptionEffBG[7];		// 옵션 이펙트 배경 이미지

// 2010. 05. 13 by jskim 신규 럭키 머신 추가방안 - 받는 아이템 이펙트 강조
	CINFImage*				m_pImgSlotDark;				// 성공시 안보이는 슬롯을 가리는 이미지

	CINFImage*				m_pImgPreArrow[3];			// 접두 화살표 이팩트 이미지
	CINFImage*				m_pImgSufArrow[3];			// 접두 화살표 이팩트 이미지
//end 2010. 05. 13 by jskim 신규 럭키 머신 추가방안 - 받는 아이템 이펙트 강조
	CINFImage*				m_pImgSlotPreQuestion;		// 접두 물음표
	CINFImage*				m_pImgSlotPreBG[2];			// 슬롯 돌아갈때 배경
	
// 2010. 05. 13 by jskim 신규 럭키 머신 추가방안 - 받는 아이템 이펙트 강조
	CINFImage*				m_pAME_slot[2];				// 받을 아이템 이팩트 강조
//end 2010. 05. 13 by jskim 신규 럭키 머신 추가방안 - 받는 아이템 이펙트 강조
	
	CINFImage*				m_pImgSlotSufQuestion;		// 접미 물음표
	CINFImage*				m_pImgSlotSufBG[2];			// 접미 슬롯 돌아갈때 배경
	
	CINFImage*				m_pImgPrizePrePositionCur;	// 접두 받을 위치 표시
	CINFImage*				m_pImgPrizeSufPositionCur;	// 접두 받을 위치 표시

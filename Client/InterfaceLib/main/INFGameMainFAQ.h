...
protected:
	...
	CINFImage*				m_pImgOper[2];						// 오퍼레이터 이미지.
	INT						m_nOperatorInfl;					// 오퍼레이터 세력.

	CINFImage*				m_pImgSearch;						// 서치 이미지.
	CINFImage*				m_pImgResult;						// 결과 배경.
	CINFImage*				m_pImgSelect;						// 제목 선택 이미지
	CINFImage*				m_pImgSelectName;					// 타이틀 선택 이미지

	CINFImage*				m_pImgButton[INF_FAQ_BUTTON_IMG];	// 버튼 이미지

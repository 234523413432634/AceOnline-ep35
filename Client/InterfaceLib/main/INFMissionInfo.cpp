...
#define MISSION_WINDOW_SIZE_WIDTH			1180							// 미션창 최대 크기 
#define MISSION_WINDOW_SIZE_HEIGHT			792							// 미션창 최대 크기
...
#define MISSION_LEFT_TEXT_GAB_X				106
#define MISSION_RIGHT_TEXT_GAB_X			648
#define MISSION_RIGHT_TEXT_GAB_Y			120
#define MISSION_FONT_HEIGHT_GAB				17

#define MISSION_SELECT_MISSION_BAR_WIDTH	486
#define MISSION_SELECT_MISSION_BAR_HEIGHT	34

#define MISSION_GIVEUP_X					1000
#define MISSION_GIVEUP_Y					110
...
#define MISSION_WARNING_OK_X				MISSION_WARNING_X + 163
#define MISSION_WARNING_OK_Y				MISSION_WARNING_Y + 136
#define MISSION_WARNING_CAN_X				MISSION_WARNING_X + 252
#define MISSION_WARNING_CAN_Y				MISSION_WARNING_Y + 136
...
HRESULT CINFMissionInfo::InitDeviceObjects()
{
	...
	for(nCount=0; nCount<IMAGE_MAX_COUNT; nCount++)
	{
		m_pMissionImg[nCount] = new CINFImage;
		pDataHeader = FindResource(buf[nCount]);
		m_pMissionImg[nCount]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	...
	for(i = 0; i < 4; i++)
	{
		...
		m_pImgOkButton[i] = new CINFImage;
		pDataHeader = FindResource(sbuf);
		m_pImgOkButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		wsprintf(sbuf, "shmcan0%d", i);
		m_pImgCancelButton[i] = new CINFImage;
		pDataHeader = FindResource(sbuf);
		m_pImgCancelButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}

	wsprintf(sbuf, "inftex");
	m_pImgWarning = new CINFImage;
	pDataHeader = FindResource(sbuf);
	m_pImgWarning->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
void CINFMissionInfo::RenderPossibleMission()
{
	...
	while(it != m_vecPossibleDesc.end())
	{
		...
		if(nLineCount+nMissionNameLineCount>=RIGHTFONT_MAX_LINE_COUNT)
		{
			m_pMissionFontRight[RIGHTFONT_MAX_LINE_COUNT]->DrawText(m_PointMissionPos.x+MISSION_RIGHT_TEXT_GAB_X,

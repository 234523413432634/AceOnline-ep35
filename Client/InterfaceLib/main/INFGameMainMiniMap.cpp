...
HRESULT CINFGameMainMiniMap::InitDeviceObjects()
{
	...
	m_pBackImage = new CINFImage;
	m_pBackImage->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	pDataHeader = m_pGameData->Find("mmsite");
	m_pSiteImage = new CINFImage;
	m_pSiteImage->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	pDataHeader = m_pGameData->Find("mmaxis");
	m_pPlayerImage = new CINFImage;
	m_pPlayerImage->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );

	// 2005-07-08 by ispark
	// 미사일 경고 이미지
	wsprintf(buf, "mwarning");
	pDataHeader = m_pGameData->Find(buf);
	m_pImgStateWarning = new CINFImage;
	m_pImgStateWarning->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);

	// 2007-02-26 by dgwoo 퀘스트 타겟에 위치 표시 이미지.
	wsprintf(buf,"qtarget");
	pDataHeader = m_pGameData->Find(buf);
	m_pImgQuestTarget = new CINFImage;
	m_pImgQuestTarget->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
	...
	for(i=0;i<3;i++)
	{
		...
		pDataHeader = m_pGameData->Find(buf);
		m_pUserImage[i] = new CINFImage;
		m_pUserImage[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
		wsprintf( buf, "mparty%d", i+1);
		pDataHeader = m_pGameData->Find(buf);
		m_pPartyImage[i] = new CINFImage;
		m_pPartyImage[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
		wsprintf( buf, "mmon%d", i+1);
		pDataHeader = m_pGameData->Find(buf);
		m_pMonsterImage[i] = new CINFImage;
		m_pMonsterImage[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
		
		wsprintf( buf, "iuser%d", i+1);
		pDataHeader = m_pGameData->Find(buf);
		m_pInfluenceImage[i] = new CINFImage;
		m_pInfluenceImage[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}
	for(i=0;i<BUTTON_STATE_NUMBER;i++)
	{
...
//		m_pButtonShowHide[BUTTON_HIDE][i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	...
	m_pDeadImage = new CINFImage;
	m_pDeadImage->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	pDataHeader = m_pGameData->Find("mlockon");
	m_pLockonImage = new CINFImage;
	m_pLockonImage->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
...
*/
	...
	{
		m_pMiniMapFrame = new CINFImage;
		m_pMiniMapFrame->InitDeviceObjects(m_MiniMapSubDataHeader[0]->m_pData, m_MiniMapSubDataHeader[0]->m_DataSize);
		m_pMiniMapFrame->RestoreDeviceObjects();
	}
	m_MiniMapSubDataHeader[1] = MiniMapData.FindFromFile("gear1");
	if(m_MiniMapSubDataHeader[1])
	{
		m_pMiniMapUnitImage = new CINFImage;
		m_pMiniMapUnitImage->InitDeviceObjects(m_MiniMapSubDataHeader[1]->m_pData, m_MiniMapSubDataHeader[1]->m_DataSize);
	...
	{
		m_pMiniMapPartyImage = new CINFImage;
		m_pMiniMapPartyImage->InitDeviceObjects(m_MiniMapSubDataHeader[2]->m_pData, m_MiniMapSubDataHeader[2]->m_DataSize);
		m_pMiniMapPartyImage->RestoreDeviceObjects();
	}
	pDataHeader = MiniMapData.FindFromFile("gear3");
	if(pDataHeader)
	{
		m_pArenaMapInfImg = new CINFImage;
		m_pArenaMapInfImg->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);

...
HRESULT CINFSpeakeMsg::InitDeviceObjects()
{
	...
	
	{
		if(NULL == m_pMsgBk)
		{
			m_pMsgBk = new CINFImage;
		}
		wsprintf(buf,"sp_bk");
		pDataHeader = FindResource(buf);
		m_pMsgBk->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}
	...
	for(nSpCnt = 0;nSpCnt < MAX_SP_IMG_STAT;nSpCnt++)
	{
		...
		{
			m_pSpImg[nSpCnt] = new CINFImage;
		}
		wsprintf(buf,"sp_icon%d", nSpCnt+1);
		pDataHeader = FindResource(buf);
		m_pSpImg[nSpCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}
	...
	// 2007-11-13 by bhsohn GM스피커 아이템 변경 
	{
		if(NULL == m_pMsgGmBk)
		{
			m_pMsgGmBk = new CINFImage;
		}
		wsprintf(buf,"sp_gmbk");
		pDataHeader = FindResource(buf);
		m_pMsgGmBk->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}
	for(nSpCnt = 0;nSpCnt < MAX_SP_IMG_STAT;nSpCnt++)
	{
		if(NULL == m_pSpGmImg[nSpCnt])
		{
			m_pSpGmImg[nSpCnt] = new CINFImage;
		}
		wsprintf(buf,"sp_gmicon%d", nSpCnt);
		pDataHeader = FindResource(buf);
		m_pSpGmImg[nSpCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}
...
{		
	...
	if(bGm == FALSE)
	{
		//2014-08-20 by St0rmy add speaker to chat
		g_pD3dApp->m_pChat->CreateChatChild(pTxt, COLOR_SPEAKER, CHAT_TAB_ALLUSER);
	}

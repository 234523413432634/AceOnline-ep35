...
HRESULT CINFSystem::InitDeviceObjects()
{
	...
	DataHeader	* pDataHeader ;
		...
		m_pSystemSelectButton[0] = new CINFImage;
	pDataHeader = FindResource("Selover");
	m_pSystemSelectButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pSystemSelectButton[1] = new CINFImage;
	pDataHeader = FindResource("Selpush");
	m_pSystemSelectButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pSystemSelectButton[2] = new CINFImage;
	pDataHeader = FindResource("Seldis");
	m_pSystemSelectButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pSystemEndButton[0] = new CINFImage;
	pDataHeader = FindResource("enover");
	m_pSystemEndButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pSystemEndButton[1] = new CINFImage;
	pDataHeader = FindResource("enpush");
	m_pSystemEndButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pSystemEndButton[2] = new CINFImage;
	pDataHeader = FindResource("endis");
	m_pSystemEndButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pLogOut = new CINFImage;
	pDataHeader = FindResource("logout");
	m_pLogOut->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pOpLog = new CINFImage;
	pDataHeader = FindResource("Op_Log");
	m_pOpLog->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pOption = new CINFImage;
	pDataHeader = FindResource("option");
	m_pOption->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pMode = new CINFImage;
	pDataHeader = FindResource("mode");
	m_pMode->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	m_pCombo->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		...
		m_pCheck = new CINFImage ;
	pDataHeader = FindResource("CHECK");
	m_pCheck->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	int i;
	for(i=0;i<OPTION_COMBO_LIST_NUMBER;i++)
	{
...
//				nWindowPosY = RIGHT_WINDOW_MAX_Y-SIZE_NORMAL_WINDOW_Y;
			...
			// 2007-07-23 by bhsohn 오퍼레이터 추가작업
			//for(i=0 ;i<OPTION_COMBO_NUMBER ;i++)
			for(int i=0 ;i<MAX_MODE_CHANGE;i++)				
			{
...
//			if(nWindowPosY > RIGHT_WINDOW_MAX_Y-SIZE_NORMAL_WINDOW_Y)
//				nWindowPosY = RIGHT_WINDOW_MAX_Y-SIZE_NORMAL_WINDOW_Y;
			for(int i=0; i<(OPTION_COMBO_NUMBER-1);i++)
			{
			...
			for(int i=1 ;i<OPTION_COMBO_NUMBER ;i++)//	if(nOption == OPTION_MODE_COMBO) 는 아직 지원하지 않는다.
			{
				int tempY = 70;
...
//				g_pInterface->SetWindowOrder(WNDLeftRightWindow);
			...
			if(pt.y>nWindowPosY + 30 && pt.y<nWindowPosY + 46)
			{
				...
				for(int i=0; i<OPTION_COMBO_NUMBER; i++){
					m_bShowOptionListBox[i] = FALSE;
				}

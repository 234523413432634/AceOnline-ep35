...
{	
	...
	DataHeader *pDataHeader = m_pGameData->Find("commubk2");
...
//		pDataHeader = m_pGameData->Find(buf);
		...
		m_pImgGuildRank[i] = new CINFImage;
		m_pImgGuildRank[i]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}
	...
	// 버튼 생성
	{
		...
		{	
			...
			if(NULL == m_pMarkBtn)
			{
				...
				m_pMarkBtn->InitDeviceObjects(szUpBtn, szDownBtn, szSelBtn, szDisBtn);		
			}			
		}
		// 탈퇴
		{	
		...
		{			
			...
			m_pImgVOIP = new CINFImage;
			m_pImgVOIP->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );			
		}
		...
		{			
			...
			m_pImgVOIPSpeaking = new CINFImage;
			m_pImgVOIPSpeaking->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );			
		}
...
void CINFCommunityGuild::RenderGuildMark(int x, int y, int nGuildUniqueNumber, int nCharacterUniqueNumber)
{
	...
	CINFImage* pMark = g_pDatabase->GetGuildMark(nGuildUniqueNumber);
	if(pMark)
	{

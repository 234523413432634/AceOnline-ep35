...
HRESULT CINFCommunityLetter::InitDeviceObjects()
{
	...
	}
	{
		...
		for(nCnt = 0;nCnt < MAX_LETTER_MODE;nCnt++)
		{
			...
			{
				m_pLetterMode[nCnt] = new CINFImage;
			}
			m_pLetterMode[nCnt]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
		}
	}
	...
	{			
		...
		{
			m_pLetterRead[LETTER_READ_NORES] = new CINFImage;
		}
		m_pLetterRead[LETTER_READ_NORES]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );

		// 읽은 편지
		pDataHeader = m_pGameData->Find("c_ler");	
		if(NULL == m_pLetterRead[LETTER_READ_RES])
		{
			m_pLetterRead[LETTER_READ_RES] = new CINFImage;
		}
		m_pLetterRead[LETTER_READ_RES]->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}
	...
	{			
		...
		{
			m_pLetterReadListBk = new CINFImage;
		}
		m_pLetterReadListBk->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}
	...
	{			
		...
		{
			m_pLetterReadBk = new CINFImage;
		}
		m_pLetterReadBk->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	}

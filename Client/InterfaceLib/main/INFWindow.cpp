...
//		sprintf(strMessageTemp, "%s %s", strText, strTime);
	...
	{
		STRING_CULL(strMessageTemp, MESSAGE_STRING_LENGTH, &vecstrTotalMessage, m_pFontInput[1]);
	}
...
HRESULT CINFMessageBox::InitDeviceObjects()
{
	...
	m_pFontMessage = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()),9, D3DFONT_ZENABLE,  FALSE,1024,32);
	m_pFontMessage->InitDeviceObjects(g_pD3dDev);
	m_pFontInput[0] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()),9, D3DFONT_ZENABLE,  FALSE,1024,32,FALSE,TRUE);
	m_pFontInput[0]->SetUV(0.0f,0.0f,67/1024.0f, 1.0f);
	m_pFontInput[0]->InitDeviceObjects(g_pD3dDev);
	
	m_pFontInput[1] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()),9, D3DFONT_ZENABLE,  FALSE,1024,32,TRUE);
	m_pFontInput[1]->InitDeviceObjects(g_pD3dDev);
	
	m_pFontRadio = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()),9, D3DFONT_ZENABLE,  FALSE,1024,32);
...
void CINFMessageBox::Render()
{
	...
	if(IS_BOLD_BK)
	{
		((CINFWindow*)m_pParent)->RenderBlackBK(m_nX, m_nY, m_nCx, m_nCy);
	}
	else
	{
		((CINFWindow*)m_pParent)->RenderCenterWindow(m_nX, m_nY, m_nCx, m_nCy);
...
HRESULT CINFWindow::InitDeviceObjects()
{
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			...
			if(pDataHeader)
			{
				m_pBoldBoxImage[i*3+j] = new CINFImage;			
				m_pBoldBoxImage[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
			}
	...
	for(i=0;i<2;i++)
	{
		for(int j=0;j<2;j++)
		{
			...
			m_pRollImage[i*2+j] = new CINFImage;
			DataHeader* pDataHeader = FindResource(buf);
			m_pRollImage[i*2+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}
	}

		m_pXclose = new CINFImage;
	DataHeader* pDataHeader = FindResource("xclose");
	m_pXclose->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pAllBox = new CINFImage;
	pDataHeader = FindResource("msgAll");
	m_pAllBox->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		m_pAllButton[0] = new CINFImage;
	pDataHeader = FindResource("all_over");
	m_pAllButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		m_pAllButton[1] = new CINFImage;
	pDataHeader = FindResource("all_push");
	m_pAllButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

// 2005-09-20 by ispark
		m_pOkButton[0] = new CINFImage;
	pDataHeader = FindResource("sok0");
	m_pOkButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pOkButton[1] = new CINFImage;
	pDataHeader = FindResource("sok1");
	m_pOkButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pOkButton[2] = new CINFImage;
	pDataHeader = FindResource("sok2");
	m_pOkButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pOkButton[3] = new CINFImage;
	pDataHeader = FindResource("sok3");
	m_pOkButton[3]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

// 2005-09-20 by ispark
		m_pCancelButton[0] = new CINFImage;
	pDataHeader = FindResource("glcan00");
	m_pCancelButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pCancelButton[1] = new CINFImage;
	pDataHeader = FindResource("glcan01");
	m_pCancelButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pCancelButton[2] = new CINFImage;
	pDataHeader = FindResource("glcan02");
	m_pCancelButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pCancelButton[3] = new CINFImage;
	pDataHeader = FindResource("glcan03");
	m_pCancelButton[3]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		
		m_pRadioButton[0] = new CINFImage;
	pDataHeader = FindResource("radio_a");
	m_pRadioButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pRadioButton[1] = new CINFImage;
	pDataHeader = FindResource("radio_b");
	m_pRadioButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	
		m_pTradeBase = new CINFImage;
	pDataHeader = FindResource("tradebs");
	m_pTradeBase->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

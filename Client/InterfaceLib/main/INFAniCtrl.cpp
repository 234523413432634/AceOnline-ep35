...
HRESULT CINFAniCtrl::InitDeviceObjects()
{
	...
	for(nInfluence = 0;nInfluence < MAX_PANI_INFLUENCE; nInfluence++)
	{
		...
		{
			m_pImageAni[nInfluence] = new CINFImage;
		}			
		pDataHeader = g_pGameMain->FindResource(chLoadFile[nInfluence]);
		m_pImageAni[nInfluence]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	...
	//2011-06-23 by jhahn 인피3차 노이즈 화면 
	{
		...
		{
			m_pInfiImageGrrr = new CINFImage;//2011-06-23 by jhahn 인피3차 노이즈 화면 
		}	
		wsprintf(chBuf, "if_grrr");
		
		pDataHeader = g_pGameMain->FindResource(chBuf);
		m_pInfiImageGrrr->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}

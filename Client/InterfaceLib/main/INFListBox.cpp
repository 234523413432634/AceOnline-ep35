...
HRESULT CINFListBox::InitDeviceObjects()
{
	...
	{
		m_pImgMain[i] = new CINFImage;
		wsprintf(buf,"%s%d",m_cMainName,i);
		pDataHeader = g_pGameMain->FindResource(buf);
		m_pImgMain[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		
	}

	m_pImgElementBG = new CINFImage;
	pDataHeader = g_pGameMain->FindResource(m_cElementBGName);
	m_pImgElementBG->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

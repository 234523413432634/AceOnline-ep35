...
public:
	...
	void RenderCenterWindow(int x, int y, int cx, int cy, BOOL bRenderClose=TRUE);
...
public:
	CINFImage*	m_pBoxImage[9];
	CINFImage*	m_pRollImage[4];
	CINFImage*	m_pXclose;
	CINFImage*	m_pOkButton[4];
	CINFImage*	m_pCancelButton[4];
	CINFImage*	m_pAllBox;
	CINFImage*	m_pAllButton[2]; // 0 : over, 1 : push
	CINFImage*	m_pRadioButton[2]; // 0 : over, 1 : push
	CINFImage*	m_pTradeBase;

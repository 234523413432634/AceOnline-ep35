...
#define CHAT_MACRO_OUTPUT_TIME		10.0f
...
//	}
	...
	{
		m_pChatViewButton[0][i] = new CINFImage;
		wsprintf(buf, "c_hsbtn0%d",i);
		pDataHeader = FindResource(buf);
		m_pChatViewButton[0][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		
		m_pChatViewButton[1][i] = new CINFImage;
		wsprintf(buf, "c_hsbtn1%d",i);
		pDataHeader = FindResource(buf);
		m_pChatViewButton[1][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pImgWisperBtn[0][i] = new CINFImage;
		wsprintf(buf, "wispbar0%d",i);
		pDataHeader = FindResource(buf);
		m_pImgWisperBtn[0][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		
		m_pImgWisperBtn[1][i] = new CINFImage;
		wsprintf(buf, "wispbar1%d",i);
		pDataHeader = FindResource(buf);
		m_pImgWisperBtn[1][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			m_pChatBox[i*3+j] = new CINFImage;
			wsprintf(buf, "c_c%d%d", i,j);
			pDataHeader = FindResource(buf);
			m_pChatBox[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;			
		}
	}
	for(i=0;i<3;i++)
	{
		m_pScrollLine[i] = new CINFImage;
		wsprintf(buf, "c_s%d", i);
		pDataHeader = FindResource(buf);
		m_pScrollLine[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	m_pChatShowImage= new CINFImage;
	pDataHeader = FindResource("Chatmage");
	m_pChatShowImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	for(i=0; i<CHAT_BUTTON_NUMBER3; i++)
	{
		m_pNChatFAQ[i] = new CINFImage;
		wsprintf(buf, "c_nfa%d", i);						// 채팅 FAQ버튼
		pDataHeader = FindResource(buf);
		m_pNChatFAQ[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		
		m_pNChatMenuView[i] = new CINFImage;
		wsprintf(buf, "c_nme%d", i);						// 채팅 메뉴 보이기 버튼
		pDataHeader = FindResource(buf);
		m_pNChatMenuView[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pNChatVOIPStartMenu[i] = new CINFImage;
		wsprintf(buf, "c_von%d", i);						// 채팅 VOIP 시작 메뉴 보이기 버튼
		pDataHeader = FindResource(buf);
		m_pNChatVOIPStartMenu[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

		m_pNChatVOIPEndMenu[i] = new CINFImage;
		wsprintf(buf, "c_vof%d", i);						// 채팅  VOIP 종료 메뉴 보이기 버튼
		pDataHeader = FindResource(buf);
		m_pNChatVOIPEndMenu[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	for(i=0; i<CHAT_BUTTON_NUMBER4; i++)
	{
		m_pNChatCash[i] = new CINFImage;
		wsprintf(buf, "c_nca%d", i);						// 채팅 채쉬충전 버튼
		pDataHeader = FindResource(buf);
		m_pNChatCash[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		
		// 2006-06-08 by ispark, 유료 상점 버튼 변경
		m_pNChatCashView[i] = new CINFImage;
//		wsprintf(buf, "cc_sh%d", i);						// 채팅 채쉬충전 버튼
		wsprintf(buf, "shbtn100%d", i);						
		pDataHeader = FindResource(buf);
		m_pNChatCashView[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	for(i = 0; i <INPUT_LANGUAGE_END;i++)
	{
		m_pNChatInputLanguage[i] = new CINFImage;
		wsprintf(buf, "lang_%d", i+1);
		pDataHeader = FindResource(buf);
		m_pNChatInputLanguage[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	for(i=0; i<CHAT_MENULIST_NUMBER; i++)
	{
		m_pNChatMenu[i] = new CINFImage;
		wsprintf(buf, "chat_li%d", i);						// 채팅 메뉴 리스트
		pDataHeader = FindResource(buf);
		m_pNChatMenu[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;									
	}

	m_pNChatInput = new CINFImage;
	pDataHeader = FindResource("c_nbar");						// 채팅 입력창
	m_pNChatInput->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	
	m_pNChatCheck = new CINFImage;
	pDataHeader = FindResource("c_nchk");						// 채팅 채크마크
	m_pNChatCheck->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
...
//		m_pBtnChatMode[ChangeChatButtonNumber(CHAT_TAB_PARTY)]->EnableBtn(FALSE);
		...
		{
			m_pChatMacroBG = new CINFImage;
		}
		pDataHeader = FindResource("chatmacro");						
		m_pChatMacroBG->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	// 음성 옵션.
	{
		...
		m_pVoiceChatBG = new CINFImage;
		pDataHeader = FindResource("voptBG");
		m_pVoiceChatBG->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

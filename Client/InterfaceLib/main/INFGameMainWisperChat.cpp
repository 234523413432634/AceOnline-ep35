...
HRESULT CINFGameMainWisperChat::InitDeviceObjects()
{
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			m_pWhisperBox[i*3+j] = new CINFImage;
			wsprintf(buf, "w_w%d%d", i,j);
			pDataHeader = FindResource(buf);
			m_pWhisperBox[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		}
	}
...
//	}
	...
	for(i=0;i<3;i++)
	{
		m_pWhisperBoxTextInput[i] = new CINFImage;
		wsprintf(buf, "wcharin0%d", i);
		pDataHeader = FindResource(buf);
		m_pWhisperBoxTextInput[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
...
void CINFGameMainWisperChat::DrawChatWindow(int x, int y, int cx, int cy)
{
	...
	{
		m_nCurrentRelScrollPosY = CHAT_BOX_TOP_IMAGE_SIZE_Y;
		m_WisperChatTab.SetRenderStartIndexWithScrollPositon(m_nWisperBoxHeight-(CHAT_BOX_TOP_IMAGE_SIZE_Y+CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y)
			, m_nCurrentRelScrollPosY-CHAT_BOX_TOP_IMAGE_SIZE_Y);
	}
	else if(m_nCurrentRelScrollPosY > cy-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y))
	{
		m_nCurrentRelScrollPosY = cy-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y);
		m_WisperChatTab.SetRenderStartIndexWithScrollPositon(m_nWisperBoxHeight-(CHAT_BOX_TOP_IMAGE_SIZE_Y+CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y)
			, m_nCurrentRelScrollPosY-CHAT_BOX_TOP_IMAGE_SIZE_Y);
	}
...
// 			m_pWhisperBoxVoice->OnMouseMove(pt);
			...
			if(m_bScrollLock)
			{
				...
				if(m_nCurrentRelScrollOldPosY+nPointGap < CHAT_BOX_TOP_IMAGE_SIZE_Y)
				{
					m_nCurrentRelScrollPosY = CHAT_BOX_TOP_IMAGE_SIZE_Y;
				}
				else if( m_nCurrentRelScrollOldPosY+nPointGap > m_nWisperBoxHeight-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y) )
				{
					m_nCurrentRelScrollPosY = m_nWisperBoxHeight-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y);
				}
				else 
...
//	m_nSystemBoxStartY				=  SYSTEM_WINDOW_START_Y;
	...
	m_nCurrentRelScrollPosY			= m_nSystemBoxHeight - (CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y);
	m_nCurrentRelScrollOldPosY		= m_nCurrentRelScrollPosY;
	m_fTwinkleTime					= WISPERBOX_NEWMESSAGE_TWINKLE_TIME;
...
HRESULT CINFSystemMsgWindow::InitDeviceObjects()
{
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			m_pSystemBox[i*3+j] = new CINFImage;
			wsprintf(buf, "w_w%d%d", i,j);
			pDataHeader = FindResource(buf);
			m_pSystemBox[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		}
	}


	m_pSystemBoxMin = new CINFImage;
	pDataHeader = FindResource("w_wMin");
	m_pSystemBoxMin->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pSystemBoxClose = new CINFImage;
	pDataHeader = FindResource("xclose");
	m_pSystemBoxClose->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	
	m_pSystemBoxName = new CINFImage;
	pDataHeader = FindResource("sysbar");
	m_pSystemBoxName->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
//				return INF_MSGPROC_BREAK;
			...
			if(m_bScrollLock)
			{
				...
				else if( m_nCurrentRelScrollOldPosY+nPointGap > m_nSystemBoxHeight-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y) )
				{
					m_nCurrentRelScrollPosY = m_nSystemBoxHeight-(CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y);
				}
				else 
				...
				m_SystemChatTab.SetRenderStartIndexWithScrollPositon(m_nSystemBoxHeight-(CHAT_BOX_TOP_IMAGE_SIZE_Y+CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y)
					, m_nCurrentRelScrollPosY-CHAT_BOX_TOP_IMAGE_SIZE_Y);
			...
			case BOTTOM_CHANGE_SIZE:	// 채팅창 사이즈 조정 Bottom
				{					 
					...
					else
					{
						...
						m_nCurrentRelScrollPosY = CHAT_BOX_TOP_IMAGE_SIZE_Y +			
							m_SystemChatTab.GetScrollPositionWithRenderStartIndex(m_nSystemBoxHeight-(CHAT_BOX_TOP_IMAGE_SIZE_Y+CHATBOX_SCROLL_IMAGE_SIZE_HEIGHT+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y)
							, (m_nSystemBoxHeight - (CHAT_BOX_TOP_IMAGE_SIZE_Y+WISPERBOX_INPUT_IMAGE_SIZE_Y+CHAT_BOX_BOTTOM_IMAGE_SIZE_Y))/CHAT_FONT_LINE_HEIGHT);
	...
	case WM_LBUTTONDOWN:
		{
			...
			if(m_bShowChatBox)
			{
				...
				else if(pt.x >= m_nSystemBoxStartX + m_nSystemBoxWidth - (CHATBOX_SCROLL_IMAGE_SIZE_WIDTH+WISPERBOX_SYSTEM_MENU_SPACE)

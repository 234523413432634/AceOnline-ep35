...
#define LUCKYMACHINE_MAIN_BG_Y					(CITY_BASE_NPC_BOX_START_Y - 285)
...
#define LUCKYMACHINE_SLOT_X						(LUCKYMACHINE_MAIN_BG_X + 45)
#define LUCKYMACHINE_SLOT_Y						(LUCKYMACHINE_MAIN_BG_Y + 36)

#define LUCKYMACHINE_SLOT_GAP_X					32
#define LUCKYMACHINE_SLOT_GAP_Y					52

#define	LUCKYMACHINE_COIN_X						(LUCKYMACHINE_MAIN_BG_X + 141)
#define	LUCKYMACHINE_COIN_Y						(LUCKYMACHINE_MAIN_BG_Y + 190)

#define ICON_SIZE_W								30
#define ICON_SIZE_H								30

#define LUCKYMACHINE_GAMBLE_BUTTON_X			(LUCKYMACHINE_MAIN_BG_X + 248)
#define LUCKYMACHINE_GAMBLE_BUTTON_Y			(LUCKYMACHINE_MAIN_BG_Y + 240)

#define LUCKYMACHINE_TAKE_BUTTON_X				(LUCKYMACHINE_MAIN_BG_X + 206)
#define LUCKYMACHINE_TAKE_BUTTON_Y				(LUCKYMACHINE_MAIN_BG_Y + 246)
...
HRESULT CINFLuckyMachine::InitDeviceObjects()
{
	...
	m_pImgMainBG[2] = new CINFImage;
	pDataHeader = FindResource("AM_BG");
	m_pImgMainBG[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pImgMainBG[1] = new CINFImage;
	pDataHeader = FindResource("WM_BG");
	m_pImgMainBG[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pImgMainBG[0] = new CINFImage;
	pDataHeader = FindResource("HM_BG");
	m_pImgMainBG[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	{	
		...
		if(pDataHeader)
		{
			m_pImgEffectBG[nCnt] = new CINFImage;		
			m_pImgEffectBG[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}	
	}	
	...
	{
		char temp[256];
		m_pImgEffBG[2][i] = new CINFImage; 
		wsprintf(temp,"AME_BG_%d",i+1);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[2][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pImgEffBG[1][i] = new CINFImage; 
		wsprintf(temp,"WME_BG_%d",i+1);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[1][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pImgEffBG[0][i] = new CINFImage; 
		wsprintf(temp,"HME_BG_%d",i+1);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[0][i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	
	for(i=0; i< 2; i++)
	{
		char temp[256];
		m_pImgEffBG[2][i+4] = new CINFImage; 
		wsprintf(temp,"AME_BG_%d",i+3);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[2][i+4]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pImgEffBG[1][i+4] = new CINFImage; 
		wsprintf(temp,"WME_BG_%d",i+3);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[1][i+4]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

		m_pImgEffBG[0][i+4] = new CINFImage; 
		wsprintf(temp,"HME_BG_%d",i+3);		
		pDataHeader = FindResource(temp);
		m_pImgEffBG[0][i+4]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	
	char temp1[256];
	m_pImgEffBG[2][6] = new CINFImage; 
	wsprintf(temp1,"AME_BG_%d",5);
	pDataHeader = FindResource(temp1);
	m_pImgEffBG[2][6]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pImgEffBG[1][6] = new CINFImage; 
	wsprintf(temp1,"WME_BG_%d",5);
	pDataHeader = FindResource(temp1);
	m_pImgEffBG[1][6]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pImgEffBG[0][6] = new CINFImage; 
	wsprintf(temp1,"HME_BG_%d",5);
	pDataHeader = FindResource(temp1);
	m_pImgEffBG[0][6]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	for(nCnt = 0;nCnt < MAX_LUCKY_YOULUCKY;nCnt++)
	{
		...
		if(pDataHeader)
		{
			m_pImgYouLuckyBG[nCnt] = new CINFImage;		
			m_pImgYouLuckyBG[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}		
	}
...
//	m_pImgArrow->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	m_pImgQuestionMark = new CINFImage;
	pDataHeader = FindResource("question");
	m_pImgQuestionMark->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	// 럭키머신 캐릭터
	{
		...
		{
			m_pImgSelctShopBK = new CINFImage;
			pDataHeader = FindResource("lbk1");
			m_pImgSelctShopBK->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}
	...
	{
		m_pImgDisableItem = new CINFImage;
		pDataHeader = FindResource("LM_inven");
		m_pImgDisableItem->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	//end 2010. 05. 12 by jskim 신규 럭키 머신 추가방안 - 럭키머신에서 받는 아이템만 보여주기
	// 2010. 05. 13 by jskim 신규 럭키 머신 추가방안 - 받는 아이템 이펙트 강조
	if(NULL == m_pAME_slot[0])
	{
		m_pAME_slot[0] = new CINFImage;
		pDataHeader = FindResource("AME_slot0");
		m_pAME_slot[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
	if(NULL == m_pAME_slot[1])
	{
		m_pAME_slot[1] = new CINFImage;
		pDataHeader = FindResource("AME_slot1");
		m_pAME_slot[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	}
...
void CINFLuckyMachine::CreateNPCListBtn()
{
	...
	for(nCnt = 0;nCnt<nLuckyMachineLen;nCnt++)
	{
		...
		if(pDataHeader)
		{
			pstruNPCList->pInfShopImage[0] = new CINFImage;
			pstruNPCList->pInfShopImage[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		...
		if(pDataHeader)
		{
			pstruNPCList->pInfShopImage[1] = new CINFImage;
			pstruNPCList->pInfShopImage[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	while(itNpcBtn != m_vecstruShopList.end())
	{
		...
		for(nCnt = 0;nCnt < MAX_SHOP_BG;nCnt++)
		{
			if(NULL == pstruNPCList->pInfShopImage[nCnt])
			{
				...
				if(pDataHeader)
				{
					pstruNPCList->pInfShopImage[nCnt] = new CINFImage;
					pstruNPCList->pInfShopImage[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
				}					
			}
		}

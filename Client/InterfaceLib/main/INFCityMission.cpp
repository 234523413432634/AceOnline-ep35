...
HRESULT CINFCityMission::InitDeviceObjects()
{
	...
	for(i=0;i<CITY_MISSION_BUTTON_NUMBER;i++)
	{
		m_pJoinButton[i] = new CINFImage;
		wsprintf( buf, "shmjoi0%d", i);
		pDataHeader = FindResource(buf);
		m_pJoinButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		m_pCompleteButton[i] = new CINFImage;
		wsprintf( buf, "shmend0%d", i);
		pDataHeader = FindResource(buf);
		m_pCompleteButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
...
//	m_pImgScroll->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	m_pImgHighLight = new CINFImage;
	pDataHeader = FindResource("shmline");
	m_pImgHighLight->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

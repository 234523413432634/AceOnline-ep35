...
HRESULT CINFCityBase::InitDeviceObjects()
{
	...
	for(i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			...
			m_pNPCBoxImage[i*3+j] = new CINFImage;
			pDataHeader = FindResource(buf);
			m_pNPCBoxImage[i*3+j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}
	}
...
//	}
	m_pUpBoxImage[0] = new CINFImage;
	pDataHeader = FindResource("sh_up0");//1x2
	m_pUpBoxImage[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	m_pUpBoxImage[1] = new CINFImage;
	pDataHeader = FindResource("sh_up1");//1x1
	m_pUpBoxImage[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pDownBoxImage[0] = new CINFImage;
	pDataHeader = FindResource("sh_base0");//1x2
	m_pDownBoxImage[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	m_pDownBoxImage[1] = new CINFImage;
	pDataHeader = FindResource("sh_base1");//1x1
	m_pDownBoxImage[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	for(i=0;i<NPC_BUTTON_NUMBER;i++)
	{
		for(int j=0;j<CITY_BUTTON_NUMBER;j++)
		{
			...
			m_pNPCButtonImage[i][j] = new CINFImage;
			pDataHeader = FindResource(buf);
			m_pNPCButtonImage[i][j]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
		}
	}
	...
	for(i = 0 ; i < CITY_BUTTON_NUMBER ; i++)
	{
		...
		m_pImgQuickAppB[i] = new CINFImage;
		pDataHeader = FindResource(buf);
		m_pImgQuickAppB[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
//	m_mapButtonGroup.clear();
	...
	// 2005-02-21 by jschoi
	m_pWebTitleBarImage = new CINFImage;
	pDataHeader = FindResource("webbar");
	m_pWebTitleBarImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	
	m_pRenewShopImage = new CINFImage;
	pDataHeader = FindResource("renewsh");
	m_pRenewShopImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
//	if( it == m_mapButtonGroup.end() )
	if( pBuilding == NULL )
	{
		...
		for(int i=0;i<CITY_BUTTON_NUMBER;i++)
		{
			pBuilding->m_pButton[i] = new CINFImage();
			char buf[32];
			wsprintf(buf, "shbtn%02d%d", info.BuildingKind, i);
			DataHeader* pDataHeader = FindResource(buf);
			if(pDataHeader)
			{
				pBuilding->m_pButton[i]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
//		(it2->second)->SetTex(pMsg->fCityWarTexRate);
		...
		// end 2010-08-31 by shcho&&jskim, 용해 시스템 구현
		g_pGameMain->ShowShopInven(m_pCurrentBuildingNPC->buildingInfo.BuildingKind, bShowInven);
		// end 2009-03-04 by bhsohn 럭키 머신 수정안		
		// end 2008-08-22 by bhsohn EP3 인벤토리 처리
	}
...
///////////////////////////////////////////////////////////////////////////////
/*
CINFImage* CINFCityBase::LoadNPCImage(int nNPCIndex)
{
	SAFE_DELETE(m_pDataHeader);
	CGameData gameData;
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFCityBase::CloseCurrentEnterBuilding()
{
	DBGOUT("CITY_NPC_BUTTON_CLOSE\n");
	/////////////////////////////////////////////////////////////////////////
	// 설정 초기화 2005-04-15 by ydkim

...
HRESULT CINFSelect::InitDeviceObjects()
{
	...
	m_pRequestEnableBack = new CINFImage;
	pDataHeader = FindResource("rq_enb");
	m_pRequestEnableBack->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);

	m_pRequestEnableTriangle = new CINFImage;
	pDataHeader = FindResource("rq_ent");
	m_pRequestEnableTriangle->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
...
//	DataHeader	* pDataHeader;
	...
	if(pDataHeader)
	{
		m_pTutorialSelectImage = new CINFImage;
		m_pTutorialSelectImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	// 2009. 10. 14 by jskim 프리스카 제거
	pDataHeader = FindResource("inf_sel");
	if(pDataHeader)
	{
		m_pInfluenceSelectImage = new CINFImage;
		m_pInfluenceSelectImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_ani3");
	if(pDataHeader)
	{
		m_pInfluenceSelectANI[0] = new CINFImage;
		m_pInfluenceSelectANI[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_ani0");
	if(pDataHeader)
	{
		m_pInfluenceSelectANI[1] = new CINFImage;
		m_pInfluenceSelectANI[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_ani1");
	if(pDataHeader)
	{
		m_pInfluenceSelectANI[2] = new CINFImage;
		m_pInfluenceSelectANI[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_bcu3");
	if(pDataHeader)
	{
		m_pInfluenceSelectBCU[0] = new CINFImage;
		m_pInfluenceSelectBCU[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_bcu0");
	if(pDataHeader)
	{
		m_pInfluenceSelectBCU[1] = new CINFImage;
		m_pInfluenceSelectBCU[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("inf_bcu1");
	if(pDataHeader)
	{
		m_pInfluenceSelectBCU[2] = new CINFImage;
		m_pInfluenceSelectBCU[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	pDataHeader = FindResource("xclose");
	if(pDataHeader)
	{
		m_pCloseBtn = new CINFImage;
		m_pCloseBtn->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}	
	//end 2009. 10. 14 by jskim 프리스카 제거
	pDataHeader = FindResource("st_dflt");
	if(pDataHeader)
	{
		m_pStartButton[0] = new CINFImage;
		m_pStartButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	pDataHeader = FindResource("st_ov");
	if(pDataHeader)
	{
		m_pStartButton[1] = new CINFImage;
		m_pStartButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	pDataHeader = FindResource("st_psh");
	if(pDataHeader)
	{
		m_pStartButton[2] = new CINFImage;
		m_pStartButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}

	pDataHeader = FindResource("skip_dft");
	if(pDataHeader)
	{
		m_pSkipButton[0] = new CINFImage;
		m_pSkipButton[0]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	
	pDataHeader = FindResource("skip_ov");
	if(pDataHeader)
	{
		m_pSkipButton[1] = new CINFImage;
		m_pSkipButton[1]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	
	pDataHeader = FindResource("skip_psh");
	if(pDataHeader)
	{
		m_pSkipButton[2] = new CINFImage;
		m_pSkipButton[2]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}
	...
	if(pDataHeader)
	{
		m_pOpNPCImage = new CINFImage;
		m_pOpNPCImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	}	
	...
	for(nCnt = 0;nCnt < MAX_UNIT_STAT;nCnt++)
	{
		...
		if(pDataHeader)
		{
			m_pUnitStatImage[nCnt] = new CINFImage;
			m_pUnitStatImage[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		}	
	}
	// 2007-07-23 by bhsohn 오퍼레이터 추가작업
	{
		...
		{		
			...
			m_pBalonChat[nCnt] = new CINFImage;
			// 오퍼레이션 에서 이미지 로드
			pDataHeader = FindResource(szImage);
			m_pBalonChat[nCnt]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;		
		}			
	}

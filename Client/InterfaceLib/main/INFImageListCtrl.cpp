...
void CINFImageListCtrl::InitDeviceEtc(char* pPlus, char* pMinus, char* pSmallIcon, char* pSelIcon)
{
	...
	// 
	{
		...
		{
			m_pListShowImage[LIST_SHOW_PLUS] = new CINFImage;
		}		
		pDataHeader = g_pGameMain->FindResource(pPlus);
		m_pListShowImage[LIST_SHOW_PLUS]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);				
	}	
	{
		if(NULL == m_pListShowImage[LIST_SHOW_MINUS])
		{
			m_pListShowImage[LIST_SHOW_MINUS] = new CINFImage;
		}		
		pDataHeader = g_pGameMain->FindResource(pMinus);
		m_pListShowImage[LIST_SHOW_MINUS]->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);				
	}
	{
		if(NULL == m_pSubItemImage)
		{
			m_pSubItemImage = new CINFImage;
		}		
		pDataHeader = g_pGameMain->FindResource(pSmallIcon);
		m_pSubItemImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);				
	}
	// 선책 상자
	{
		if(NULL == m_pSelItemImage)
		{
			m_pSelItemImage = new CINFImage;
		}		
		pDataHeader = g_pGameMain->FindResource(pSelIcon);
		m_pSelItemImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);				
	}
...
void CINFImageListCtrl::InserItem(int nEpId, int nMainIdx, char* pItemName, BOOL bEpTitle)
{
	...
	pImageListCtrlIcon->pInfImage = new CINFImage;	

	DataHeader	* pDataHeader = NULL;
	pDataHeader = g_pGameMain->FindResource(pItemName);
	pImageListCtrlIcon->pInfImage->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);		

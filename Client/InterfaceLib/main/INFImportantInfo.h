...
public:
	...
	DWORD			m_nMapNum;
	CINFImage*		m_pImgMapState[MAP_STATE_NUMBER];			// 맵의 상태에 따른 이미지.
	CINFImage*		m_pImgHP;					// 피 바 이미지.
	CINFImage*		m_pImgHPBg;					// 피 배경 이미지.
	CINFImage*		m_pImgReadyTel;				// 준비상태의 텔레포트 이미지.
	CINFImage*		m_pImgSuccTel;				// 활성화 완료 텔레포트 이미지.
	CINFImage*		m_pImgAttWarning;			// 경고 이미지.
	
	CINFImage*		m_pActAni[TELEPORT_ACT_ANIMATION];			// 활성화 진행중 애니매이션.
	UINT			m_nAniNum;									// 애니매이션 이미지 번호.
	float			m_fAniTime;									// 시간 누적.
	BOOL			m_bActAni;									// 활성화 진행중인 애니매이션.

	CINFImage*		m_pActCoolTime[TELEPORT_ACT_COOLTIME];		// 활성화 진행효과를 칸이 차는듯한 이미지.

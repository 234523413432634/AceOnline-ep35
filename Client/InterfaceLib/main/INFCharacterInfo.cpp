...
//	m_pScrollBar = NULL;
	...
	m_pFontStatDec = NULL;
	for(int i=0 ; i<CHARACTER_INTERFACE_FONT_NUM ; i++)
	{
	...
	for(int i=0;i<C_QUEST_LIST_NUM;i++)
	{
		m_pFontQuestList[i] = NULL;
//		m_pFontQuestState[i] = NULL;
	}
	for(int i=0;i<C_QUEST_DESC_NUM;i++)
	{
		m_pFontQuestDesc[i] = NULL;
	}
	for(int i=0; i<CHAR_UTC_SLOT_NUMBER; i++)
	{
...
HRESULT CINFCharacterInfo::InitDeviceObjects()
{
	...
	m_pInfo->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
		...
		m_pInfoLog = new CINFImage;
	pDataHeader = FindResource("in_log");
	m_pInfoLog->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;

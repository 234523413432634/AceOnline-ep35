...
HRESULT CINFInfinity::InitDeviceObjects()
{
	...
	for( i=0; i<10; ++i )
	{
		...
		if ( pHeader )
		{
			m_pTimeNumImage[ i ] = new CINFImage;
			m_pTimeNumImage[ i ]->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}	
	}
	...
	if( m_pColonImage == NULL )
	{
		...
		if( pHeader )
		{
			m_pColonImage = new CINFImage;
			m_pColonImage->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pClearImage == NULL )
	{
		...
		if( pHeader )
		{
			m_pClearImage = new CINFImage;
			m_pClearImage->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pFailImage == NULL )
	{
		...
		if( pHeader )
		{
			m_pFailImage = new CINFImage;
			m_pFailImage->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pTenderBG == NULL )
	{
		...
		if( pHeader )
		{
			m_pTenderBG = new CINFImage;
			m_pTenderBG->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pTenderProgressFullBar == NULL )
	{
		...
		if( pHeader )
		{
			m_pTenderProgressFullBar = new CINFImage;
			m_pTenderProgressFullBar->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pTenderProgressHalfBar == NULL )
	{
		...
		if( pHeader )
		{
			m_pTenderProgressHalfBar = new CINFImage;
			m_pTenderProgressHalfBar->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	for( i=0; i<10; ++i )
	{
		...
		if( pHeader  )
		{
			m_pTenderTime[ i ] = new CINFImage;
			m_pTenderTime[ i ]->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}	
	}
...
// 	}
	...
	if( m_pHPBG == NULL )
	{
		...
		if( pHeader )
		{
			m_pHPBG = new CINFImage;
			m_pHPBG->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pHP == NULL )
	{
		...
		{
			m_pHP = new CINFImage;
			m_pHP->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pWarning == NULL )
	{
		...
		if( pHeader )
		{
			m_pWarning = new CINFImage;
			m_pWarning->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	if( m_pIMG_Label_Step == NULL )
	{
		...
		if( pHeader )
		{
			m_pIMG_Label_Step = new CINFImage;
			m_pIMG_Label_Step->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		}
	}
	...
	for ( i = 0; i < 10; ++i )
	{
		if ( NULL == m_pIMG_Label_BNum[i] )
		{
			...
			if ( pHeader )
			{
				m_pIMG_Label_BNum[i] = new CINFImage;
				m_pIMG_Label_BNum[i]->InitDeviceObjects ( pHeader->m_pData , pHeader->m_DataSize );
			}
		}
		...
		if ( NULL == m_pIMG_Label_SNum[i] )
		{
			...
			if ( pHeader )
			{
				m_pIMG_Label_SNum[i] = new CINFImage;
				m_pIMG_Label_SNum[i]->InitDeviceObjects ( pHeader->m_pData , pHeader->m_DataSize );
			}
		}
	}
...
// 		m_pTenderNo->RestoreDeviceObjects();
	...
	std::map< INT, CINFImage* >::iterator it = m_mapKeyMonsterImg.begin();
...
// 		m_pTenderNo->InvalidateDeviceObjects();
	...
	std::map< INT, CINFImage* >::iterator itT = m_mapKeyMonsterImg.begin();
...
// 	}
	...
	while( it != m_vecTenderItemList.end() )
	{
		...
		// 입찰 버튼
		(*it)->m_pTenderYes->SetBtnPosition( bkPos.x + 44, bkPos.y + 151 );
		(*it)->m_pTenderYes->Render();

		// 포기 버튼
		(*it)->m_pTenderNo->SetBtnPosition( bkPos.x + 175, bkPos.y + 151 );
...
void	CINFInfinity::SetTipImg( char* szTipFileName )
{
	...
	if( pHeader )
	{
		m_pTipImg = new CINFImage;
		m_pTipImg->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
		m_pTipImg->RestoreDeviceObjects();
...
void	CINFInfinity::AddKeyMonsterImg( MEX_MONSTER_INFO* pMonsterInfo )
{
		...
		&& m_mapKeyMonsterImg.find( pMonsterInfo->MonsterUnitKind ) == m_mapKeyMonsterImg.end() )
	{
		...
		if( pHeader )
		{
			CINFImage* pImg = new CINFImage;
			pImg->InitDeviceObjects( pHeader->m_pData, pHeader->m_DataSize );
			pImg->RestoreDeviceObjects();

			m_mapKeyMonsterImg.insert( std::pair<INT, CINFImage*>( pMonsterInfo->MonsterUnitKind, pImg ) );
		}
	}
}
...
void	CINFInfinity::RenderKeyMonster( void )
{
	...
	{
		std::map<INT, CINFImage*>::iterator monImgIt = m_mapKeyMonsterImg.find( (*keyMonit)->m_pMonsterInfo->MonsterUnitKind );
		...
		case WM_MOUSEMOVE:
			{
				...
				else
				{
					...
					{
						if( (*it)->m_pItemInfo )
							g_pGameMain->SetItemInfoUser( (*it)->m_pItemInfo, pt.x, pt.y, FALSE );
					}
					else
						g_pGameMain->SetItemInfoNormal( NULL, 0, 0 );
				}

...
{	
	...
	// end 2009. 05. 07 by ckPark 럭키머신 버튼 수정
	CINFImage*			pInfShopImage[MAX_SHOP_BG];
	BOOL				bReStore;	
...
protected:
	...
	CINFImage*				m_pImgQuestionMark;				// 물음표 
//	CINFImage*				m_pImgArrow;					// 화살표.
	
	CINFImage*				m_pImgMainBG[LUCKY_SELECT_MAX];						// 메인 배경.

	CINFImage*				m_pImgEffectBG[MAX_LUCKY_TITLE_IMAGE];				// 메인 위의 배경.

	CINFImage*				m_pImgEffBG[LUCKY_SELECT_MAX][7];

	CINFImage*				m_pImgYouLuckyBG[MAX_LUCKY_YOULUCKY];				// 축하 배경

	// 2010. 05. 12 by jskim 신규 럭키 머신 추가방안 - 럭키머신에서 받는 아이템만 보여주기
	CINFImage*				m_pImgDisableItem;				// 사용 아이템 그림자

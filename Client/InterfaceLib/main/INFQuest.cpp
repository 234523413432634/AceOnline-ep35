...
HRESULT CINFQuest::InitDeviceObjects()
{
	...
	{
		char buf[64];
		m_pImageQuestButton[i] = new CINFImage;
		wsprintf( buf, "qquest%d", i);
		pDataHeader = FindResource(buf);
		m_pImageQuestButton[i]->InitDeviceObjects(pDataHeader->m_pData, pDataHeader->m_DataSize);
...
CINFImage* CINFQuest::LoadNPCImage(int nNPCIndex)
{
	SAFE_DELETE(m_pDataHeader);
	CGameData gameData;

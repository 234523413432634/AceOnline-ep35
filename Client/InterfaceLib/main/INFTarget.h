...
public:
	CINFImage*		m_pCenter[2];				// "t_center0, t_center1", 가운데 표시
	CINFImage*		m_pRedMouse;				// "t_m_red", 마우스 빨간색 타겟
	CINFImage*		m_pArrowMouse;				// "t_m_whi", 마우스 하얀색 방향지시
	CINFImage*		m_pBlueMouse;				// "t_m_blue", 마우스 파란색 타겟
	CINFImage*		m_p2stLockOn;				// "t_2st_l", 2형 무기 락온
	CINFImage*		m_pTextLockOn;				// "t_text_l", "LOCKON!"텍스트
	CINFImage*		m_pAutoTarget;				// "t_ototar", 자동 타겟 설
	CINFImage*		m_pAutoTargetLong;			// "t_oto", 자동 타겟 (사정거리 밖에 있는 경우
	CINFImage*		m_pSkillTarget;				// "skTarget", 타겟이 있는 스킬 사용시 타겟
	CINFImage*		m_pHPBox;					// "t_hpbox", HP박스
	CINFImage*		m_pHP;						// "t_hp", HP정보
	CINFImage*		m_pNormalMouse[2];			// "t_m_red", 마우스 빨간색 타겟
	CINFImage*		m_pFireAngle[2];			// "t_angle", 화망 (화망이미지,A기어는 안나옴)	

	CINFImage*		m_pWisperBoxLeftTRightB;	// 귓말 채팅창 마우스 이미지
	CINFImage*		m_pWisperBoxLeftBRightT;	// 귓말 채팅창 마우스 이미지
	CINFImage*		m_pWisperBoxLeftRight;		// 귓말 채팅창 마우스 이미지
	CINFImage*		m_pWisperBoxTopBottom;		// 귓말 채팅창 마우스 이미지
	CINFImage*		m_pWisperBoxCross;			// 귓말 채팅창 마우스 이미지

	CINFImage*		m_pMonTargetStateImg;		// 몬스터의 공격대상 상태 표시하는 이미지(몬스터의 이름 양쪽)
	CINFImage*		m_pImgTargetMe[2];			// 내가 타겟을 잡은 몬스터
...
//	CAppEffectData		* m_pTargetDirArrow;	// 화면에 벗어나는 타겟 방향 화살표
	...
	CINFImage*		m_pTargetDirArrow[4];		// 화면에 벗어나는 타겟 방향 화살표
	// end 2010. 03. 03 by ckPark 인피니티 필드 2차(입장 UI변경)
	CINFImage*		m_pImgArrowHelp;			// 세력선택 전까지 화살표의 설명을 해준다.
...
protected:
	...
	CINFImage*		m_pImgTargetHelper[2];				// 2007-07-27 by dgwoo 저렙을 위한 설명 [0] 1형, [1] 2형.
	CINFImage*		m_pImgSelectTarget[2];
	CINFImage*		m_pImgPartyTarget[2];				// 2008-06-17 by dgwoo 파티원을 타겟잡은 몬스터의 표시.

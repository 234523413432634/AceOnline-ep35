...
HRESULT CINFGameArena::InitDeviceObjects()
{
	...
	m_pOperNor = new CINFImage;
	m_pOperNor->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	wsprintf(buf,"operdie");
	pDataHeader = FindResource(buf);
	m_pOperDie = new CINFImage;
	m_pOperDie->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	wsprintf(buf,"opersel");
	pDataHeader = FindResource(buf);
	m_pOperSel = new CINFImage;
	m_pOperSel->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	wsprintf(buf,"operbcu");
	pDataHeader = FindResource(buf);
	m_pOperBCU = new CINFImage;
	m_pOperBCU->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	wsprintf(buf,"operani");
	pDataHeader = FindResource(buf);
	m_pOperANI = new CINFImage;
	m_pOperANI->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
	wsprintf(buf,"hp_bar");
	pDataHeader = FindResource(buf);
	...
	wsprintf(buf,"dp_bar");
	pDataHeader = FindResource(buf);
	m_pImgDp = new CINFImage;
	m_pImgDp->InitDeviceObjects( pDataHeader->m_pData, pDataHeader->m_DataSize );
...
{ 
	...
	SetArenaGameState(ARENA_GAME_STATE_SCORE);
 	g_pScene->ChangeObjectBodyCondition(ARENA_DOOR_OBJECT,BODYCON_DOOR_OPENING_MASK);
